...
bool CTableLotterySetting::DisuseIssue(int issue, int state)
{
	...
	if (DBNODATA(sExec))
	{
		ToLogService("common", "lottery couldn't find the issue {}!", issue);
		return false;
	}
...
bool CTableAmphitheaterSetting::DisuseSeason(int season, int state,const char* winner)
{
	...
	if (DBNODATA(sExec))
	{
		ToLogService("common", "amphitheater couldn't find the season {}!", season);
		return false;
	}
...
bool CTableAmphitheaterTeam::TeamSignUP(int &teamID, int captain, int member1, int member2)
{
	...
	if (DBNODATA(sExec))
	{
		ToLogService("common", "amphitheater couldn't find the id {}!", teamID);
		return false;
	}
...
bool CTableResource::ReadKitbagTmpData(long lRecDBID, string& strData)
{
	...
	{
		ToLogService("store", "Unknown Exception raised when ReadKitbagTmpData");
	}
