...
void Player::EndPlay(net::TcpClient *client)
{
	if(m_currcha >=0)
	{
		...
		{
			net::RPacket l_rpk1;
			g_gpsvr->CP_TEAM_LEAVE(this,client,l_rpk1);
		}
		if(m_chatarranum)
		{
			...
			while(m_chatarranum)
			{
				net::WPacket l_wpk2(256);
				l_wpk2.WriteInt64(m_chat[0]->GetSession()->GetID());
				auto rpkt = net::RPacket(l_wpk2.Data(), l_wpk2.GetPacketSize());
				g_gpsvr->CP_SESS_LEAVE(this,client,rpkt);
			}
		}
		if (Guild* guild = GetGuild(); guild)
		{
			...
			{
				net::WPacket l_toGuild(256);
				l_toGuild.WriteCmd(CMD_PC_GUILD);
				l_toGuild.WriteInt64(MSG_GUILD_OFFLINE);
				l_toGuild.WriteInt64(m_chaid[m_currcha]);
		...
		}
		{
			...
			//Friends offline notification
			{
				...
				l_toFrnd.WriteInt64(MSG_FRND_REFRESH_OFFLINE);
				l_toFrnd.WriteInt64(m_chaid[m_currcha]);
...

			{
				...
				l_toFrnd.WriteInt64(MSG_PRENTICE_REFRESH_OFFLINE);
				l_toFrnd.WriteInt64(m_chaid[m_currcha]);
			...
			
			{
				...
				net::WPacket	l_toFrnd(256);
				l_toFrnd.WriteCmd(CMD_PC_MASTER_REFRESH);
				l_toFrnd.WriteInt64(MSG_MASTER_REFRESH_OFFLINE);
				l_toFrnd.WriteInt64(m_chaid[m_currcha]);
		...
		if (m_gm > 0)
		{
			...
			wpk.WriteInt64(MSG_FRND_REFRESH_DEL);
			wpk.WriteInt64(m_chaid[m_currcha]);
...
{
	if(m_currcha <0 || !m_gate || !m_gate->IsValid())return;
...
void Player::DoCommand(cChar *cmd)
{
	...
	}else if(!strncmp(cmd,"userconnection ",strlen("userconnection ")))
	{
		...
		if(!l_ply)
		{
			//SendSysInfo(dstring("????")<<cmd +strlen("getuserconnection ") <<"???2?????");
			char l_buf[512];
	...
	}else if(!strcmp(cmd ,"TotalOn"))
	{
		...
		if(l_ply)
		{
			if(l_ply->m_gm > 0)
			{
				...
				{
					//SendSysInfo(dstring("????/????yo")<<dwLoginNum<<"/"<<dwPlayerNum);
					char l_buf[512];
	...
	}else if(!strncmp(cmd ,"pingx ",strlen("pingx ")))
	{
		...
		if(!l_ply)
		{
			//SendSysInfo(dstring("????")<<cmd +strlen("ping ") <<"???2?????");
			char l_buf[512];
	...
	}else if( !strncmp( cmd, "mutex", strlen("mutex") ) )
	{
		...
		l_cmd = strchr( l_cmd, ' ' );
		//if( !l_cmd ) SendSysInfo( "???????o@@ estop [??????????3?],[???????????]" );
...
void Player::DisablePlayer( cChar* plyname, uLong lTimes )
{
	...
	else
	{
		...
		else
		{
			...
			net::WPacket l_wpk(256);
			l_wpk.WriteCmd(CMD_PA_USER_DISABLE);
			l_wpk.WriteInt64(m_acctLoginID);
			l_wpk.WriteInt64(lTimes);
			g_gpsvr->m_acctClient.Send(l_wpk);
