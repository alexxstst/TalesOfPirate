...
PtInviter Invitation::BeginInvited(Player * inviter)
{
	...
	// TODO: invitation timeout checking     GetProcessor/m_plyproc
	l.unlock();
	return l_ptinviter;
}
...
PtInviter Invitation::EndInvited(uLong atorID)
{
	PtInviter	l_ptinviter	=0;
	RunChainGetArmor<Invited> l(*this);
	Invited	*	l_invited;
	while((l_invited=GetNextItem())&&l_invited->m_ptinviter.m_chaid !=atorID){}
	if(l_invited)
	{
		l_ptinviter	=l_invited->m_ptinviter;
		if(l_invited->_EndRun()==1)
...
Invited *Invitation::FindInvitedByInviterChaID(uLong atorID)
{
	Invited	*l_invited	=0;
	RunChainGetArmor<Invited> l(*this);
	while((l_invited=GetNextItem())&&l_invited->m_ptinviter.m_chaid !=atorID){}
