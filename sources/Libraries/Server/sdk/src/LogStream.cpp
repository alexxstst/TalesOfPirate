...
void LogStream::Open(bool bakmode) {
	...
	l_path[0] = 0;
	if (!m_DirName.empty()) {
		if (m_DirName[0] == '/' || m_DirName[0] == '\\') {
			strcpy(l_path, m_DirName.substr(1).c_str());
		}
		else {
			strcpy(l_path, m_DirName.c_str());
		}
		int l_dircount = 0;
		cChar* l_dir = l_path;
		char *l_dim1, *l_dim2, *l_dim;
		while ((l_dim1 = (char*)strchr(l_dir, '/')), (l_dim2 = (char*)strchr(l_dir, '\\')), (l_dim =
				min(l_dim1, l_dim2)),
			...
			*l_dim = 0;
			if (strlen(l_dir) > 0) {
				if (l_dircount == 1) {
					...
					SYSTEMTIME l_st;
					::GetLocalTime(&l_st);
					sprintf(l_olddir, l_frmstr, l_st.wYear, l_st.wMonth, l_st.wDay, l_st.wHour, l_st.wMinute,
							l_st.wSecond, l_st.wMilliseconds);
		...
		if (strlen(l_dir) > 0) {
			if (l_dircount == 1) {
				...
				SYSTEMTIME l_st;
				::GetLocalTime(&l_st);
				sprintf(l_olddir, l_frmstr, l_st.wYear, l_st.wMonth, l_st.wDay, l_st.wHour, l_st.wMinute, l_st.wSecond,
						l_st.wMilliseconds);
	...
	strcat(l_path, m_LogFileName.c_str());
	strcat(l_path, ".log");
	if (bakmode) {
...
void LogStream::Backup() {
	...
	try {
		RunChainGetArmor<LogStream> l_lockMgr(*s_logmgr);
		while (l_logs = s_logmgr->GetNextItem()) {
			MoveFile(l_logs->m_DirRoot.c_str(), (l_logs->m_DirRoot + l_bakdir).c_str());
		}
		l_lockMgr.unlock();
	}
	catch (...) {
...
_DBC_BEGIN
	...
	std::ostream& endln(std::ostream& of) {
		try {
			...
			std::cout << "log endln exception" << std::endl;
		}
		return of;
	}
