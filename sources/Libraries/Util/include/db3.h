...
public:
	...
	bool operator <(TDt& dt2) {
		return Julian() < dt2.Julian();
	}
...
public:
	...
	void SetWhere(const char* columnName, const char* value, char a_DataType = STRING, bool a_bUseLike = false,
				  bool a_bUseOr = false);
	void SetWhere(const char* columnName, const std::string& value, char a_DataType = STRING, bool a_bUseLike = false,
				  bool a_bUseOr = false);
...
public:
	...
	void SetColumn(const char* columnName, bool isValidTime, int month, int day, int year);
	// use CSQLBASE_OLEDATETIME( o ) as 2nd arg
