...
inline int CRawDataSet::GetRawDataID(const char* pszDataName) // ID,
{
	...
	if (it != _IDIdx.end()) // ID
	{
		...
		if (_nUnusedIndex >= _nIDCnt) {
			ToLogService("errors", LogLevel::Error, "RawDataSet OverMax Dynamic ID, MaxIDCnt = {}, Index = {}", _nIDCnt,
						 _nUnusedIndex);
			return -1;
		}
...
inline BOOL CRawDataSet::_LoadRawDataInfo_Txt(const char* pszFileName, int nSep) {
	...
	while (!in.eof()) {
		...
		if (n > _nMaxFieldCnt) {
			//LG2("error", "msg[%s]\n", pszFileName);
			ToLogService("errors", LogLevel::Error, " in resource [{}], the field num is greater than predefine count ",
						 pszFileName);
			bRet = FALSE;
			break;
		}
		...
		if (!bSaveFieldCnt) {
			...
			if (nFieldCnt != n) {
				...
				ToLogService("errors", LogLevel::Error,
							 " parse resource file [{}] failed ,No [{}], please chech format and version!", pszFileName,
							 pstrList[0].c_str());
		...
		if (!IsValidID(nID)) {
			//LG2("error", "msg[%d][%s]\n", nID, pszFileName);
			ToLogService("errors", LogLevel::Error, " index [{}] overflow,please check resource file [{}]", nID,
						 pszFileName);
			bRet = FALSE;
			break;
		}
