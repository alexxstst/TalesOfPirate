...
public:
	...
	int GetRawDataID(const std::string& pszDataName);
	...
	//
	void SetReleaseInterval(DWORD dwInterval) {
		_dwReleaseInterval = dwInterval;
	}

	void SetMaxRawData(int nDataCnt) {
		_nMaxRawDataCnt = nDataCnt;
	}

	int GetLoadedRawDataCnt() {
		return _nLoadedRawDataCnt;
	}
	...
	void EnableRequest(BOOL bEnable) {
		_bEnableRequest = bEnable;
	}
...
inline void* CRawDataSet::GetRawData(int nID, BOOL bRequest) {
	...
	if (!pInfo->pData) {
		if (bRequest && _bEnableRequest) {
			ToLogService("debug", "Push Request RawData!");
		...
		pInfo->dwLoadCnt++;
		if (pInfo->pData == NULL) {
			ToLogService("error", "Load Raw Data [{}] Failed! (ID = {})", pInfo->szDataName, nID);
		}
		else {
			_nLoadedRawDataCnt++;
		}
...
inline int CRawDataSet::GetRawDataID(const std::string& pszDataName) // ID,
{
	CRawDataInfo* pInfo;
	...
	if (it != _IDIdx.end()) // ID
	{
		...
		if (_nUnusedIndex >= _nIDCnt) {
			ToLogService("error", "RawDataSet OverMax Dynamic ID, MaxIDCnt = {}, Index = {}", _nIDCnt, _nUnusedIndex);
			return -1;
		}

		LPBYTE pbtData = (LPBYTE)_RawDataArray;
		pInfo = (CRawDataInfo*)(pbtData + _GetRawDataInfoSize() * _nUnusedIndex);
		strcpy(pInfo->szDataName, pszDataName.c_str());
...
inline BOOL CRawDataSet::_LoadRawDataInfo_Txt(const char* pszFileName, int nSep) {
	...
	while (!in.eof()) {
		...
		if (n > _nMaxFieldCnt) {
			//LG2("error", "msg[%s]\n", pszFileName);
			ToLogService("error", "msg in resource [{}], the field num is greater than predefine count",
						 pszFileName);
			bRet = FALSE;
			break;
		}
		...
		if (!bSaveFieldCnt) {
			...
			if (nFieldCnt != n) {
				//
				//LG2("error", "msg[%s],[%s], !\n",
				ToLogService("error", "msg parse resource file [{}] failed ,No [{}], please chech format and version!",
		...
		if (!IsValidID(nID)) {
			//LG2("error", "msg[%d][%s]\n", nID, pszFileName);
			ToLogService("error", "msg index [{}] overflow,please check resource file [{}]", nID, pszFileName);
			bRet = FALSE;
			break;
		}
...
//   xxx.bin
//--------------------------------------------
inline void CRawDataSet::PackFromDirectory(std::list<std::string>& DirList, const char* pszPackName,
	...
	for (std::list<std::string>::iterator it = FileList.begin(); it != FileList.end(); it++, i++) {
		...
		ToLogService("debug", "Pack File (index = {}) ID = {} [{}]", pInfo->nIndex, pInfo->nID, pInfo->szDataName);
	}
