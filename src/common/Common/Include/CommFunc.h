...
inline const char* g_GetServerError( int error_code )  // 
{
	...
	case ERR_PT_BADBOY:	return "My child, you are very bold. You have been reported to the authority. Please do not commit the offense again!";// "BT";
	...
	default:
		{
			...
			switch ( l_error_code )
			{
			case ERR_MC_BASE:return strcat(strcpy(l_buffer,_itoa(error_code,l_convt,10)),"(GameServer/GateServer->Client return error code space 1-500)"); //"(GameServer/GateServer->Client1500)");
			case ERR_PT_BASE: return strcat(strcpy(l_buffer, _itoa(error_code,l_convt,10)),"(GroupServer->GateServer return error code range 501-1000)"); //"(GroupServer->GateServer5011000)"); 
			case ERR_AP_BASE: return strcat(strcpy(l_buffer, _itoa(error_code,l_convt,10)),"(AccountServer->GroupServe return error code from 1001-1500)"); //"(AccountServer->GroupServer10011500)");
			case ERR_MT_BASE: return strcat(strcpy(l_buffer, _itoa(error_code,l_convt,10)),"(GameServer->GateServer return error code range 1501-2000)"); //"(GameServer->GateServer15012000)");
			default:return strcat(strcpy(l_buffer, _itoa(error_code,l_convt,10)),"(Jack is too insane, he made a mistake that I don\'t even know.)"); //"(JackBT)");
			}
		}
	}
}
...
#define eTableMax 5
	...
	*/
	...
	CTextFilter();
	~CTextFilter();
	static bool Add(const eFilterTable eTable, const std::string& szFilterText);
	...
	static bool LoadFile(const std::string& szFileName, const eFilterTable eTable=NAME_TABLE);
