...
inline std::int64_t to_integerx(lua_State* L, int idx, int* isnum)
{
    int ok = 0;
    ...
    if (ok)
    {
        const auto int_n = static_cast<std::int64_t>(n);
...
inline std::int64_t tointeger(lua_State* L, int idx, int* isnum)
{
#if ! LUABRIDGE_ON_LUAU && LUA_VERSION_NUM > 502
