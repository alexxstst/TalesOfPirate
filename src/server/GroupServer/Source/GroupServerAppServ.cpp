...
bool GroupServerApp::OnConnect(DataSocket* datasock) //???:true-?D??,false-2??D??
{
	datasock->SetRecvBuf(64 * 1024);
	datasock->SetSendBuf(64 * 1024);
	if (!datasock->IsServer()) {
		ToLogService("Connect", "AccountServer:{},{} come ! Socket num:{}", datasock->GetPeerIP(),
					 datasock->GetPeerPort(), GetSockTotal() + 1);

		std::cout << "AccountServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() << "come ! Socket num:"
			<< GetSockTotal() + 1 << std::endl;
	}
	else {
		ToLogService("Connect", "GateServer:{},{} come ! Socket num:{}", datasock->GetPeerIP(), datasock->GetPeerPort(),
					 GetSockTotal() + 1);

		std::cout << "GateServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() << "come ! Socket num:" <<
			GetSockTotal() + 1 << std::endl;
	}
	return true;
}
...
void GroupServerApp::OnDisconnect(DataSocket* datasock, int reason) {
	if (!datasock->IsServer()) {
		...
		m_mtxlogin.unlock();
		/*
		l_line<<newln<<"AccountServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"???Sockety:"
			<<GetSockTotal()<<"reason=("<<reason<<")"<<GetDisconnectErrText(reason)
			<<endln;
		*/
		ToLogService("Connect", "AccountServer:{},{} go ! Socket num:{}reason=({}){}", datasock->GetPeerIP(),
					 datasock->GetPeerPort(), GetSockTotal(), reason, GetDisconnectErrText(reason));
		//std::cout<<"AccountServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"???Sockety:"<<GetSockTotal()<<"reason="<<GetDisconnectErrText(reason).c_str()<<std::endl;
		std::cout << "AccountServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() << "go ! Socket num:"
			<< GetSockTotal() << "reason=" << GetDisconnectErrText(reason).c_str() << std::endl;
		...
		m_mtxlogin.unlock();
		/*
		l_line<<newln<<"GateServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"???Sockety:"
			<<GetSockTotal()<<"reason=("<<reason<<")"<<GetDisconnectErrText(reason)
			<<endln;
		*/
		ToLogService("Connect", "GateServer:{},{} go ! Socket num:{}reason=({}){}", datasock->GetPeerIP(),
					 datasock->GetPeerPort(), GetSockTotal(), reason, GetDisconnectErrText(reason));
		//std::cout<<"GateServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"???Sockety:"<<GetSockTotal()<<"reason="<<GetDisconnectErrText(reason).c_str()<<std::endl;
		std::cout << "GateServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() << "go ! Socket num:" <<
			GetSockTotal() << "reason=" << GetDisconnectErrText(reason).c_str() << std::endl;
...
void GroupServerApp::OnProcessData(DataSocket* datasock, RPacket& recvbuf) {
	try {
		...
		ToLogService("OnProcessData", "CMD: {}", l_cmd);
		switch (l_cmd) {
		...
		ToLogService("packetException", "Packet CMD [{}] from [{}] caused exception.", recvbuf.ReadCmd(),
					 datasock->GetPeerIP());
		return;
	}
}
...
WPacket GroupServerApp::OS_LOGIN(DataSocket* datasock, RPacket& pk) {
	...
	ToLogService("Connect", "AgentServer:{},{} OS_LOGIN1", datasock->GetPeerIP(), datasock->GetPeerPort());

	m_mtxAgent.lock();

	ToLogService("Connect", "AgentServer:{},{} OS_LOGIN2", datasock->GetPeerIP(), datasock->GetPeerPort());
	try {
		if (m_groupServerAgent.SetDataSock(datasock)) {
			l_retpk.WriteShort(ERR_SUCCESS);
			std::cout << "AgentServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() <<
	...
	m_mtxAgent.unlock();
	ToLogService("GroupServer", "AgentServer:{}:{} OS_LOGIN3", datasock->GetPeerIP(), datasock->GetPeerPort());
	...
	try {
		...
		if (l_gate) {
			if (l_gate->SetDataSock(datasock)) {
				l_retpk.WriteShort(ERR_SUCCESS);
				//std::cout<<"GateServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"?????31|?"<<std::endl;
				std::cout << "GateServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() <<
					"re login success!" << std::endl;
			}
			else {
				l_retpk.WriteShort(ERR_PT_SAMEGATENAME);
				//std::cout<<"GateServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"???:?????GateServer?????"<<std::endl;
				std::cout << "GateServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() <<
			...
			++m_gatenum;
			//std::cout<<"GateServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"???31|?"<<std::endl;
			std::cout << "GateServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() << "login success" <<
				std::endl;
		}
		else {
			l_retpk.WriteShort(ERR_PT_LOGFAIL);
			//std::cout<<"GateServer:"<<datasock->GetPeerIP()<<","<<datasock->GetPeerPort()<<"????"<<std::endl;
			std::cout << "GateServer:" << datasock->GetPeerIP() << "," << datasock->GetPeerPort() << "login failed!" <<
				std::endl;
			Disconnect(datasock);
		}
	}
			...
			if (!player->m_chaid[i]) {
				...
				if ((l_row = m_tblcharaters->FetchRowByChaID(player->m_chaid[i])) == 1) {
					...
					try {
						...
						ToLogService("GroupServer", "enum account [{}]'s char[ID:{}] find appearance data exception.",
									 player->m_acctname, player->m_chaid[i]);
						i--;
						continue;
					}
				}
					...
					if (l_row == 0) {
						...
						ToLogService("GroupServer",
									 "enum account[{}]'s char[ID:{}] can't find data in table character.",
									 player->m_acctname, player->m_chaid[i]);
					}
					else {
						//l_line<<newln<<"????o?["<<player->m_acctname<<"]????[ID:"<<player->m_chaid[i]<<"]oy?Y?a2??";
						ToLogService("GroupServer", "enum account[{}]'s char[ID:{}] databse exception.",
									 player->m_acctname, player->m_chaid[i]);
					}
					i--;
				}
	...
	if ((l_row = m_tblaccounts->FetchRowByActName(player->m_acctname.c_str())) == 0) {
		...
		for (int i = 0; i < player->m_chanum; i++) {
			if ((l_row = m_tblcharaters->FetchRowByChaID(player->m_chaid[i])) == 1) {
				...
				if (l_row == 0) {
					ToLogService("GroupServer", "CanCreateBonusChar:{}'s charID:{} can't find data in table character.",
								 player->m_acctname, player->m_chaid[i]);
				}
				else {
					ToLogService("GroupServer", "CanCreateBonusChar:{}'s charID:{} databse exception.",
								 player->m_acctname, player->m_chaid[i]);
				}
				return false;
			}
...
char GroupServerApp::SendCharData(Player* player, WPacket& wpk) {
	...
	try {
		...
		ToLogService("GroupServer", "SendCharData: appearance data convert exception");
		wpk.WriteChar(0);
		return -1;
	}
...
bool GroupServerApp::GetCHAsFromDBByPlayer(Player* player, WPacket& wpk) {
	...
	if ((l_row = m_tblaccounts->FetchRowByActName(player->m_acctname.c_str())) == 0) //D?????o?
	{
		...
		player->m_chanum =
			char(Util_ResolveTextLine(m_tblaccounts->GetChaIDs(), l_chaid, Player::emMaxCharacters, ';'));
		wpk.WriteChar(player->m_chanum); //?????y
		...
		ToLogService("GroupServer", "enum account[{}] operate table account failed", player->m_acctname);
	}
	return true;
}
...
WPacket GroupServerApp::TP_USER_LOGIN(DataSocket* datasock, RPacket& pk) {
	...
	RPacket rpkFromAccountServer = SyncCall(m_acctsock, l_wpk, l_ulMilliseconds); //?AccountServer??
	if (!rpkFromAccountServer.HasData()) {
		auto l_retpk = GetWPacket();
		l_retpk.WriteShort(ERR_PT_NETEXCP); //???
		ToLogService("GroupServer", "({}):[{}] login net failed", l_ply->m_clientip, l_ply->m_acctname);
		l_ply->Free();
		return l_retpk;
	}
	...
	if (l_errno) {
		...
		ToLogService("GroupServer", "({}):[{}]login failed, error:{}", l_ply->m_clientip, l_ply->m_acctname, l_errno);
		l_ply->Free();
		return l_retpk;
	}
	...
	auto l_retpk = GetWPacket();
	l_retpk.WriteShort(ERR_SUCCESS); //31|???
	...
	l_retpk.WriteLong(ToAddress(l_ply)); //??????o???
	l_ply->BeginRun();
	//l_line<<newln<<"("<<l_ply->m_clientip<<"):["<<l_ply->m_acctname<<"]???31|?\t????/????y:"<<m_plylst.GetTotal()<<"/"<<long(m_curChaNum)<<endln;
	ToLogService("GroupServer", "({}):[{}]login success, 	 nline/total:{}/{}", l_ply->m_clientip, l_ply->m_acctname,
...
void GroupServerApp::AP_KICKUSER2(DataSocket* datasock, uLong acctid) {
	...
	if (l_ply) {
		...
		if (l_ply->EndRun()) {
			...
			ToLogService("GroupServer", "recieved killed acctid/acctname:[{}]/[{}] command!", l_acctid,
						 l_ply->m_acctname);
		}
		l_ply->Free();
	}
		...
		ToLogService("GroupServer", "recieved kill acctid:[{}] command(not in play list)!", l_acctid);
	}
}
...
void GroupServerApp::AP_KICKUSER(DataSocket* datasock, RPacket& pk) {
	...
	if (l_ply) {
		...
		if (l_ply->EndRun()) {
			...
			ToLogService("GroupServer", "recieved killed acctid/acctname:[{}]/[{}] command!", l_ply->m_acctid,
						 l_ply->m_acctname);
		}
		l_ply->Free();
	}
...
WPacket GroupServerApp::TP_USER_LOGOUT(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	if (ply->EndRun()) {
		...
		ToLogService("GroupServer", "({}):[{}]logout,	 online/total:{}/{}", ply->m_clientip, ply->m_acctname,
					 m_plylst.GetTotal(), (long)m_curChaNum);
	}
	else {
		ToLogService("GroupServer", "({}):[{}]{}{}/{}", ply->m_clientip, ply->m_acctname,
...
WPacket GroupServerApp::TP_BGNPLAY(Player* ply, DataSocket* datasock, RPacket& pk) {
	WPacket l_retpk = GetWPacket();
	if (ply->m_currcha >= 0) {
		l_retpk.WriteShort(ERR_PT_INERR); //|??????2??
		return l_retpk;
	}
	...
	l_line<<newln<<"("<<ply->m_clientip<<"):["<<ply->m_acctname<<"]???????["<<l_cha
		...
		<<long(m_curChaNum);
	*/
	ToLogService("GroupServer", "({}):[{}]select char[{}]begin paly...?	 online/total:{}/{}", ply->m_clientip,
				 ply->m_acctname, ply->m_chaname[ply->m_bp_currcha], m_plylst.GetTotal(), (long)m_curChaNum);
	return l_retpk;
}
...
WPacket GroupServerApp::TP_ENDPLAY(Player* ply, DataSocket* datasock, RPacket& pk) {
	WPacket l_retpk = GetWPacket();
	if (ply->m_currcha < 0) {
		l_retpk.WriteShort(ERR_PT_INERR); //|??????32?????
	}
	else {
		l_retpk.WriteShort(ERR_SUCCESS); //???????31|??
		ply->EndPlay(datasock);

		ToLogService("GroupServer", "({}):[{}]exit char[{}]begin entry select char UI...?	 online/total:{}/{}",
					 ply->m_clientip, ply->m_acctname, ply->m_chaname[ply->m_currcha], m_plylst.GetTotal(),
					 (long)m_curChaNum);
...
void GroupServerApp::MP_ONLINE(Player* ply) {
	...
	++m_curChaNum;
	ToLogService("GroupServer", "({}):[{}] char [{}]entry map server begin play...?	 online/total:{}/{}",
				 ply->m_clientip, ply->m_acctname, ply->m_chaname[ply->m_currcha], m_plylst.GetTotal(),
				 (long)m_curChaNum);
...
WPacket GroupServerApp::TP_NEWCHA(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	*/
	...
	else if (l_len == 0 || !isAlphanumeric({l_chaname, l_len}) || !IsValidName(l_chaname, l_len) || !
		CTextFilter::IsLegalText(CTextFilter::NAME_TABLE, l_chaname)) //(strchr(l_chaname,'\''))//??????D2??Dyo?'
	{
		l_retpk.WriteShort(ERR_PT_ERRCHANAME); //?????2?o?
		return l_retpk;
	}
	...
	//{
	//	l_retpk.WriteShort(ERR_PT_INVALIDBIRTH);		//3???
	//}else
	//{
	cChar* l_birth = pk.ReadString(&l_len);
	std::string l_birth1 = "";
	...
	try {
		...
		//l_line<<newln<<"?o?["<<ply->m_acctname<<"]D?????["<<l_chaname<<"]oa1?y?Ya??3?"<<endln;
		ToLogService("GroupServer", "account[{}]new char[{}]find appreance data convert exception", ply->m_acctname,
					 l_chaname);
		l_retpk.WriteShort(ERR_PT_INVALIDDAT);
		return l_retpk;
	}
	{
		...
		{
			ToLogService("GroupServer", "account[{}]new char[{}]the same name exception", ply->m_acctname, l_chaname);
			l_retpk.WriteShort(ERR_PT_SAMECHANAME);
			return l_retpk;
		}
		l_retpk.WriteShort(ERR_SUCCESS);
		...
		if (m_tblcharaters->FetchRowByChaName(l_chaname) < 1) {
			l_retpk.WriteShort(ERR_PT_INERR);
			ToLogService("GroupServer", "account[{}] when create char , can't restore database failed.",
						 ply->m_acctname);
			return l_retpk;
		}
		...
		for (char i = 0; i < ply->m_chanum; i++) {
			strcat(l_CharIDs, _itoa(ply->m_chaid[i], l_buf, 10));
			strcat(l_CharIDs, ";");
		}
	...
	*/
	ToLogService("GroupServer", "({}):[{}]create char [{}]?	 online/total:{}/{}", ply->m_clientip, ply->m_acctname,
				 l_chaname, m_plylst.GetTotal(), (long)m_curChaNum);
	return l_retpk;
}
...
WPacket GroupServerApp::TP_DELCHA(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	if (bFlag) {
		l_retpk.WriteShort(ERR_SUCCESS); //?3y???31|??
		--ply->m_chanum;
		...
		l_line<<newln<<"("<<ply->m_clientip<<"):["<<ply->m_acctname<<"]?3y????["
			...
			<<endln;
	   */
		ToLogService("GroupServer", "({}):[{}] select char [{}] begin play, online/total:{}/{}", ply->m_clientip,
					 ply->m_acctname, cha_name.c_str(), m_plylst.GetTotal(), (long)m_curChaNum);
	}
	else {
		l_retpk.WriteShort(ERR_PT_SERVERBUSY); //t????|?????o
		/*
		l_line<<newln<<"("<<ply->m_clientip<<"):["<<ply->m_acctname<<"]?3y???["
			<<l_cha<<"]?\t????/????y:"<<m_plylst.GetTotal()<<"/"<<long(m_curChaNum)
			<<endln;
		*/
		ToLogService("GroupServer", "({}):[{}]delete char failed[{}]?	 GetTotal/m_curChaNum:{}/{}", ply->m_clientip,
					 ply->m_acctname, cha_name.c_str(), m_plylst.GetTotal(), (long)m_curChaNum);
	}
	return l_retpk;
}
...
WPacket GroupServerApp::TP_CREATE_PASSWORD2(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	if (ply->m_password.length() == 0 && strPassword.length() <= ROLE_MAXSIZE_PASSWORD2 && IsValidName(
		...
		if (m_tblaccounts->UpdatePassword(ply->m_acctid, strPassword.c_str())) {
			...
			l_retpk.WriteShort(ERR_SUCCESS);
			ToLogService("ErrorServer", "account:{}ID:{}database operate success:({})", ply->m_acctname, ply->m_acctid,
						 strPassword.c_str());
		}
		else {
			l_retpk.WriteShort(ERR_PT_SERVERBUSY);
			ToLogService("ErrorServer", "account:{}ID:{}database operate failed:({})", ply->m_acctname, ply->m_acctid,
						 strPassword.c_str());
		}
	}
	else {
		l_retpk.WriteShort(ERR_PT_INVALID_PW2);
		ToLogService("ErrorServer", "account:{}ID:{}wrong second password:({})", ply->m_acctname, ply->m_acctid,
					 strPassword.c_str());
	}
	return l_retpk;
}
...
*/
/*							char	l_tmpbuf[20];
							strcpy(l_look,_itoa(look->sTypeID,l_tmpbuf,10));
							for(short i=0;i<enumEQUIP_NUM;i++)
							{
								strcat(l_look,";");
								strcat(l_look,_itoa(look->chState[i],l_tmpbuf,10));
							}
							for(short i=0;i<enumEQUIP_NUM;i++)
							{
								strcat(l_look,";");
								strcat(l_look,_itoa(look->sLink[i],l_tmpbuf,10));
							}
*/
...
void GroupServerApp::MP_GARNER2_UPDATE(Player* ply, DataSocket* datasock, RPacket& pk) {
	...
	if (strChaName.length() > 19) {
		//l_line<<newln<<...;
		ToLogService("Garner2", "order data exception");
		return;
	}
