...
WPacket	GroupServerApp::TP_NEWCHA(Player *ply,DataSocket *datasock,RPacket &pk)
{
	...
	*/
	...
	else if (l_len == 0 || !isAlphanumeric({ l_chaname,l_len }) || !IsValidName(l_chaname, l_len) || !CTextFilter::IsLegalText(CTextFilter::NAME_TABLE, l_chaname))								//(strchr(l_chaname,'\''))//??????D2??Dyo?'
	{
