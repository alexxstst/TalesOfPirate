...
public:
	...
	dbc::Short GetWidth() {
		return m_sUnitCountX;
	}

	dbc::Short GetHeight() {
		return m_sUnitCountY;
	}
...
public:
	...
	bool IsOpen(void) {
		return m_bValid && m_chState == enumMAP_STATE_OPEN;
	}
	...
	void SetCopyPlyNum(dbc::Short sPlyNum) {
		m_sCopyPlyNum = sPlyNum;
	}

	dbc::Short GetCopyPlyNum(void) {
		return m_sCopyPlyNum;
	}
	...
	bool HasDynEntry(void) {
		return strcmp(m_szEntryMapName, "") != 0 ? true : false;
	}
	...
	void SetCopyStartType(dbc::Char chStartType = enumMAPCOPY_START_PLAYER) {
		m_chCopyStartType = chStartType;
	}

	dbc::Char GetCopyStartType(void) {
		return m_chCopyStartType;
	}
	...
	dbc::Char GetCopyStartCdtType(void) {
		return m_chCopyStartCdtType;
	}

	dbc::Long GetCopyStartCdtVal(void) {
		return m_lCopyStartCdtVal;
	}

	void SetName(dbc::cChar* cszName) {
		m_strMapName = cszName;
	}
	...
	void SetRepatriateDie(bool bRepatriate = true) {
		m_bRepatriateDie = bRepatriate;
	}

	bool IsRepatriateDie(void) {
		return m_bRepatriateDie;
	}
