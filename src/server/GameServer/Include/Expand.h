...
inline int lua_AddState(lua_State* pLS) {
...

	{
		...
		if (!pCTarCha->AddSkillState(g_uchFightID, pCSrcCha->GetID(), pCSrcCha->GetHandle(), enumSKILL_TYPE_SELF,
									 enumSKILL_TAR_LORS, enumSKILL_EFF_HELPFUL, uchStateID, uchStateLV, nOnTime,
									 enumSSTATE_ADD_UNDEFINED, false)) {
#ifdef defPARSE_LOG
			//g_pCLogObj->Log("\t[%d%d]\n", uchStateID, SKILL_STATE_MAXID);
			g_pCLogObj->Log(
				"\tAdding the state to Main program failed [Adding uchStateID %d,SKILL_STATE_MAXID %d] ,transfer failed",
				uchStateID, SKILL_STATE_MAXID);
#endif
			bSuccess = false;
			goto End;
...
End:
	if (bSuccess) {
		...
		g_pCLogObj->Log("%s add Skill state to %s succeed : state %d,grade %d,duration %d\n", pCSrcCha->GetLogName(),
						pCTarCha->GetLogName(), uchStateID, uchStateLV, nOnTime);
#endif
	}
...
inline int lua_RemoveState(lua_State* pLS) {
	...
	}
	{
		...
		if (!pCCha->DelSkillState(uchStateID)) {
			...
			//g_pCLogObj->Log("\t[%d%d]\n", uchStateID, SKILL_STATE_MAXID);
			g_pCLogObj->Log(
				"\tDelete the state to Main program failed[uchStateID %d,SKILL_STATE_MAXID %d],transfer failed",
				uchStateID, SKILL_STATE_MAXID);
#endif
			bSuccess = false;
			goto End;
...
inline int lua_SkillUnable(lua_State* pLS) {
	...
	}
	{
		...
		ToLogService("skill failed", "role's name {}", pCCha->GetLogName());
	}
End:
	if (bSuccess) {
#ifdef defPARSE_LOG
...
inline int lua_AddChaSkill(lua_State* pLS) {
	...
	}
	{
		...
		if (!pCCha->LearnSkill((short)nSkillID, (char)nSkillLv, bSetLv, bUsePoint, checkReq)) {
			...
			g_pCLogObj->Log(
				"\tAdding the Skill to Main program failed[nSkillID %d,nSkillLv %d,defMAX_SKILL_NO %d],transfer failed\n",
				nSkillID, nSkillLv, defMAX_SKILL_NO);
#endif
			bSuccess = false;
			goto End;
...
inline int lua_EndGuildChallenge(lua_State* pLS) {
	...
	game_db.EndChall((DWORD)lua_tonumber(pLS, 1), (DWORD)lua_tonumber(pLS, 2),
					 (DWORD)lua_tonumber(pLS, 3) ? TRUE : FALSE);
