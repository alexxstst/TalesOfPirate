...
{T_B
	...
	catch (...)
	{
		if (!GetPlayer())
			...
			LG("exception3", "character player[name %sDatabaseID %u]release occured abnormity, [CCharacter::Finally]\n", GetLogName(), GetPlayer()->GetDBActId());
		throw;
	}
T_E}
...
{T_B
	...
	if (!strcmp(pCSrcMap->GetName(), szTarMapName)) // 
	{
		if (GetPlayer())
			...
			LG("enter_map", "SwitchMap(the same map switchcontrol player name %s[mainplayer %s]mapname %s)--------\n", GetLogName(), GetPlyMainCha()->GetLogName(), szTarMapName);
	...
	else
	{
		...
		LG("enter_map", "SwitchMap(differ Server map switchcontrol player name %s[mainplayer %s]formerly map %saimmap %s)--------\n", GetLogName(), GetPlyMainCha()->GetLogName(), pCSrcMap->GetName(), szTarMapName);
		if (GetSubMap())
			//LG("enter_map", " %s\n", GetSubMap()->GetName());
...
{T_B
	...
	if (m_pCPlayer && pCCha->m_pCPlayer && (GetID() == pCCha->GetID()))
		...
		LG("eyeshot error", "the homonymy player %s out of eyeshottheir socket%p%p.\n", pCCha->GetLogName(), m_pCPlayer->GetGate(), pCCha->m_pCPlayer->GetGate());
...
{T_B
	...
	m_CLog.Log("start study skill:skillID %dlevel %dwhether set level %d.\n", sSkillID, chLv, bSetLv);
...
bool CCharacter::CanLearnSkill(CSkillRecord *pCSkill, Char chToLv)
{
	...
	if (pCSkill->sLevelDemand > m_CChaAttr.GetAttr(ATTR_LV)) // 
	{
		...
		m_CLog.Log("can't study:character level not enough%dremand level %d.\n", m_CChaAttr.GetAttr(ATTR_LV), pCSkill->sLevelDemand);
		return false;
	}
		...
		for( int i = 0; i < sNumGird; i++ )
		{
			...
			if( pGridCont )
			{
				...
				if( pItem == NULL )
				{
					...
					//LG( "boat_error", "ID!ID = %d", pGridCont->sID );
					LG( "boat_error", "GridID errorcan't find the gridID = %d", pGridCont->sID );
					continue;
				}
