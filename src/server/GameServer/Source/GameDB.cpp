...
{T_B
	...
	{
		//LG("enter_map", "Player.\n");
		LG("enter_map", "Loading database errorPlayer is empty.\n");
		return false;
	}
	CCharacter *pCha = pPlayer->GetMainCha();
	...
	if (DBOK(r) && r1 > 0)
	{
		...
		try
		{
			...
			if (!pCha->String2LookDate(g_buf[nIndex++]))
			{
				...
				LG("Check sum error", "the character (dbid %uname %sresid %u)'s change appearance data check sum error.\n", atorID, pCha->GetLogName(), pCha->GetKitbagRecDBID());
				return false;
			}
	...
	else
	{
		...
		LG("enter_map", "Loading database error_get_row() return value%d.%u\n", r, r1);
		return false;
	}
...
{T_B
	...
	if (pPlayer->GetLoginChaType() == enumLOGIN_CHA_BOAT) // 
	{
		...
		if (pCLogCha != pCCtrlCha) // 
		{
			...
			if (pCLogCha)
				...
				LG("logging character control error", "logging character %s,control character %sMain character %s.\n", pCLogCha->GetLogName(), pCCtrlCha->GetLogName(), pCha->GetLogName());
			else
	...
	if( !pPlayer->MisGetTrigger( g_szTrigger, ROLE_MAXSIZE_DBTRIGGER - 1 ) )
	{
		...
		//LG(szDBLog, "[ID: %d\tNAME: %s]!ID = %d\n", atorID, pCha->GetName(), pCha->GetID() );
		LG(szDBLog, "save character[ID: %d\tNAME: %s]data infoGet mission trigger data error!ID = %d\n", atorID, pCha->GetName(), pCha->GetID() );
	}
	//LG("enter_map", "3.\n");
	...
	if( !pPlayer->MisGetMissionCount( g_szMisCount, ROLE_MAXSIZE_DBMISCOUNT - 1 ) )
	{
		...
		//LG(szDBLog, "[ID: %d\tNAME: %s]!ID = %d\n", atorID, pCha->GetName(), pCha->GetID() );
		LG(szDBLog, "save character[ID: %d\tNAME: %s]data infoGet randomicity mission take count of data error!ID = %d\n", atorID, pCha->GetName(), pCha->GetID() );
	}
	//LG("enter_map", "4.\n");
...
{T_B
	...
	if( !pPlayer->MisGetTrigger( g_szTrigger, ROLE_MAXSIZE_DBTRIGGER - 1 ) )
	{
		...
		LG(szDBLog, "SaveMissionData:save character[ID: %d\tNAME: %s]data infoGet mission trigger data error!ID = %d\n", atorID, pCha->GetName(), pCha->GetID() );
...
bool CTableResource::ReadKitbagData(CCharacter *pCha)
{
	...
	if (DBOK(r) && r1 > 0)
	{
		...
		{
			//LG("enter_map", ".\n");
			LG("enter_map", "Load resource database errorcharacter is not matching.\n");
			return false;
		}
	...
	else
	{
		...
		LG("enter_map", "Load resource database error_get_row()return value:%d.\n", r);
		return false;
	}
	//LG("enter_map", ".\n");
...
{T_B
    ...
    if(!r)
    {
		...
		LG("gamedb", "msgDatabase [%s] Connect Failed!, ERROR REPORT[%d]", g_Config.m_dsn.c_str(), err_info.c_str() );
        return FALSE;
    }
	C_PRINT("sucess!\n");
...
bool CGameDB::SavePlayer(CPlayer *pPlayer, char chSaveType)
{
	...
	if (pPlayer->GetMainCha()->GetPlayer() != pPlayer)
	{
		...
		LG("save character great error", "save Player address %p[dbid %u]the character main player %sthe character 's Player address %p\n",		
	...
	try
	{
		...
		LG("save data waste time", "totalize %-8dmain character %-8dmain character kitbag %-8dbank %-8dbig map %-8dboat %-8d.[%d %s]\n",
...
bool CTableGuild::ChallWin( BOOL bUpdate, BYTE byLevel, DWORD dwWinGuildID, DWORD dwFailerGuildID )
{
	...
	else
	{
		...
		else
		{
			...
			LG( "challenge consortia", "finish challenge consortialeizhu failed:inquire about failed consortia level info failed!GUILDID = %d, WINID = %d.\n", dwFailerGuildID, dwWinGuildID );
			return false;
		}
...
void CTableGuild::ChallMoney( BYTE byLevel, BOOL bChall, DWORD dwGuildID, DWORD dwChallID, DWORD dwMoney )
{
	...
	else
	{
		...
		LG( "challenge consortia result", "challenge succeedwinner:ID = %d,loser:ID = %d, money = %u,level:%d.\n", dwChallID, dwGuildID, dwMoney, byLevel  );
