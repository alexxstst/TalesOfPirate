...
static auto split2 = [](const std::string& line, char delim)
...
bool CGameConfig::Load(char* pszFileName) {
	...
	while (!in.eof()) {
		...
		auto trimLine = strLine;
		Util_TrimString(trimLine);
		if (trimLine.empty()) {
			continue;
		}
		...
		int n = Util_ResolveTextLine(trimLine.c_str(), strPair, 2, '=');
		if (n < 2)
			continue;
		...
		else if (strKey == "info") // InfoServer IPport
		{
			...
			size_t stPos = 0;
			if ((stPos = strValue.find("0x")) != std::string::npos || (stPos = strValue.find("0X")) !=
				std::string::npos) //
