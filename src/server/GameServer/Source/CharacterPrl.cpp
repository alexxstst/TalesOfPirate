...
void CCharacter::ProcessPacket(unsigned short usCmd, RPACKET pk) {
	...
	case CMD_CM_SAY: {
		...
		if (!GetSubMap()) {
			ToLogService("dialog error", "when character{} is dialogthe map is null", GetLogName());
			break;
		}
		uShort l_retlen;
		...
		/*else if (*l_content == '/' && *(l_content+1)=='?') //
		{
			...
			g_CParser.DoString("HandleChat", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this,
							   enumSCRIPT_PARAM_STRING, 1, l_content, DOSTRING_PARAM_END);
	...
	case CMD_CM_GUILD_PUTNAME: {
		...
		if (l_guildname && Guild::IsValidGuildName(l_guildname, uShort(strlen(l_guildname))) && l_passwd && !strchr(
			l_passwd, '\'')) {
	...
	case CMD_CM_GUILD_MOTTO: {
		...
		if (l_motto && strlen(l_motto) < 50 && IsValidName(l_motto, uShort(strlen(l_motto)))) {
			...
			if (canMotto == emGldPermMotto && !strchr(l_motto, '\'')) {
				// Probably not enough
...
void CCharacter::BeginAction(RPACKET pk) {
	...
	case enumACTION_UPDATEGUILDLOGS: {
		...
		for (int i = 1; i <= 13; i++) {
			if (i > logs.size()) {
				// We reached the end, send signal to stop
				WRITE_SHORT(WtPk, 9);
				break;
			}
	...
	case enumACTION_REQUESTGUILDLOGS: {
		...
		for (int i = 1; i <= 13; i++) {
			...
			if ((int)(curSize + i) > logs.size()) {
				// We reached the end of logs before fetching those 13 logs, send stop parameter
				WRITE_SHORT(WtPk, 9);
				break;
			}
