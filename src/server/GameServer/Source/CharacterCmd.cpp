		...
		setAttr(ATTR_SP, 1);
	{
...

		{
			...
			else
			{
				if (chLogin == 0) // 
				{
					if (strcmp(l_map, pCCtrlCha->GetBirthMap()))
					{
						...
						LG("enter_map", "character %s(%s)'s aim map %s is not matched to focus character %sbe cut off connect!\n",
			...
			if (!pCMapRes)
			{
				...
				LG("enter_map", "player %s(%s)'s map name or city name is unlawfulbe cut off connect!\n", GetLogName(), pCCtrlCha->GetLogName());
				sErrCode = ERR_MC_ENTER_POS;
				goto Error;
			}
			...
			{
				//LG("enter_map", " %s(%s) !\n", GetLogName(), pCCtrlCha->GetLogName());
				LG("enter_map", "character %s(%s) copy map ID is unlawfulbe cut off connect!\n", GetLogName(), pCCtrlCha->GetLogName());
				sErrCode = ERR_MC_ENTER_POS;
				goto Error;
			}
...
if (!IsLiveing()) {
	...
	for (Char chPCount = 0; chPCount < chPointNum; chPCount++)
		if (!m_submap->IsValidPos(pPath[chPCount].x, pPath[chPCount].y))
		{
			...
			//m_CLog.Log("\n");
			m_CLog.Log("move path errorpathID is unlawful\n");
			return;
		}
...
{T_B
	...
	if (!pSSkillCont)
	{
		...
		if (!pSSkillCont)
		{
			...
			//m_CLog.Log("[SkillID: %u] [PacketID: %u]\n", pSkill->sID, m_ulPacketID);
	...
	for (Char chPCount = 0; chPCount < chPointNum; chPCount++)
		if (!m_submap->IsValidPos(pPath[chPCount].x, pPath[chPCount].y))
		{
			FailedActionNoti(enumACTION_MOVE, enumFACTION_MOVEPATH);
			//m_CLog.Log("\n");
			m_CLog.Log("move path errorpath ID is unlawful\n");
	...
	if (SkillTarIsEntity(pSkill)) // ID
	{
		...
		{
			//m_CLog.Log("[PacketID: %u]\tTick %u\n", m_ulPacketID, GetTickCount());
			m_CLog.Log("the same skill requestbe ignore [PacketID: %u]\tTick %u\n", m_ulPacketID, GetTickCount());
	...
	if (!IsPlayerCha())
	{
		...
		{
			//m_CLog.Log("[PacketID: %u]\tTick %u\n", m_ulPacketID, GetTickCount());
			m_CLog.Log("the same skill and aimreset move path[PacketID: %u]\tTick %u\n", m_ulPacketID, GetTickCount());
			return;
		}
	}
...
{T_B
	...
	{
		//m_CLog.Log("ID: %u\n", lSkillNo);
		m_CLog.Log("skillID: %uis inexistence\n", lSkillNo);
		return;
	}
...
{T_B
	...
	{
		//m_CLog.Log("ID: %u\n", lSkillNo);
		m_CLog.Log("skillID: %u's object isn't the coordinate point\n", lSkillNo);
		return;
	}
...
{T_B
	...
	if( pItem->sType == enumItemTypeBoat )
	{
		...
		if (SaveAssets())
		{
			...
			{
				//LG( "", "%sID[0x%X]!ID[0x%X]", 
				LG( "pick up goods error", "character%sID[0x%X]pick up captain provebut boat data storage failed!boat data ID[0x%X]", 
		...
		{
			//LG( "", "%sID[0x%X]!ID[0x%X]", 
			LG( "pick up goods error", "character%sID[0x%X]pick up captain provebut kitbag data storage failed!boat data ID[0x%X]", 
				this->GetName(), this->GetPlayer()->GetDBChaId(), dwBoatID );
		}
		...
		if( !BoatAdd( dwBoatID ) )
		{
			...
			//LG( "", "%sID[0x%X]!ID[0x%X]", 
			LG( "pick up goods error", "character%sID[0x%X]pick up captain proveadd boat failed!boat dataID[0x%X]", 
				this->GetName(), this->GetPlayer()->GetDBChaId(), dwBoatID );
		}
	}
