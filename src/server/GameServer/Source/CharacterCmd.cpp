		...
		setAttr(ATTR_SP, 1);
	{
...

		{
			...
			for (int i = 0; i < enumEQUIP_NUM; i++) {
				...
				if (m_SChaPart.SLink[i].sID && (m_SChaPart.SLink[i].expiration - std::time(0)) <= 0 && m_SChaPart.SLink[
					i].expiration != 0) {
					Short sUnfixNum = 0;
			...
			//
			if (bNewCha)
				g_CParser.DoString("CreatCha", enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this,
								   DOSTRING_PARAM_END);
			else
			...
			//WriteKitbag(*m_pCKitbagTmp, pkret, enumSYN_KITBAG_INIT); // !!!entermap

			WRITE_CHAR(pkret, chLogin);
			WRITE_LONG(pkret, g_pGameApp->m_dwPlayerCnt);
			WRITE_LONG(pkret, ToAddress(pCPlayer)); //GateServerPlayer
...
Short CCharacter::Cmd_UseEquipItem(Short sKbPage, Short sKbGrid, bool bRefresh, bool rightHand) {
	...
	if (CItemRecord::IsVaildFusionID(pItem) && pCItemRec->sType != enumItemTypeBoat && pSEquipIt->GetFusionItemID())
	// ning.yan  end
	{
...
Short CCharacter::Cmd_UseExpendItem(Short sKbPage, Short sKbGrid, Short sTarKbPage, Short sTarKbGrid, bool bRefresh) {
	...
	if (strcmp(pCItemRec->szAttrEffect, "0") == 0) {
		...
		g_CParser.DoString(pCItemRec->szAttrEffect, enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 3, this,
						   pSGridCont, pSTarGridCont, DOSTRING_PARAM_END);
	...
	char szMsg[128];
	//sprintf(szMsg, " %s[%u] %u.", pCItemRec->szName, SGridCont.sID, SGridCont.sNum);
...
// 2
//=============================================================================
Short CCharacter::Cmd_UnfixItem(Char chLinkID, Short* psItemNum, Char chDir, Long lParam1, Long lParam2, bool bPriority,
								bool bRefresh, bool bForcible) {
	...
	if (chDir == 0) //
	{
		...
		if (!pCMap)
			return enumITEMOPT_ERROR_KBLOCK;
		pCMap->ItemSpawn(&SUnfixCont, lParam1, lParam2, enumITEM_APPE_THROW, pCCtrlCha->GetID(), pCMainCha->GetID(),
						 pCMainCha->GetHandle());
		...
		if (IsBoat())
			...
			sprintf(szPlyName, "%s", GetName());
		char szMsg[128];
		//sprintf(szMsg, " %s[%u] %u.", pCItemRec->szName, SUnfixCont.sID, *psItemNum);
...
Short CCharacter::Cmd_PickupItem(uLong ulID, Long lHandle) {
	...
	if (pCItem->GetProtChaID() != 0) //
	{
		if (pCItem->GetProtChaID() != pCMainCha->GetID()) {
			...
			if (pBelongEnt) {
				...
				if (pBelongPlayer && (!pBelongPlayer->getTeamLeaderID() || pBelongPlayer->getTeamLeaderID() !=
					GetPlayer()->getTeamLeaderID())) {
					return enumITEMOPT_ERROR_PROTECT;
				}
			}
		}
	...
	if (IsBoat())
		...
		sprintf(szPlyName, "%s", GetName());
	char szMsg[128];
	//sprintf(szMsg, " %s[%u] %u.", pItem->szName, pCItem->m_SGridContent.sID, sPickupNum);
	...
	char szTeamMsg[128]{};
	sprintf_s(szTeamMsg, GM_CHARACTERCMD_CPP_00006, szPlyName, sPickupNum, pItem->szName);
...
//
// psThrowNum 0
Short CCharacter::Cmd_ThrowItem(Short sKbPage, Short sKbGrid, Short* psThrowNum, Long lPosX, Long lPosY, bool bRefresh,
								bool bForcible) {
	if (GetPlyMainCha()->m_CKitbag.IsPwdLocked()) //
		return enumITEMOPT_ERROR_KBLOCK;
	...
	*psThrowNum = GridCont.sNum;
	...
	if (IsBoat())
		...
		sprintf(szPlyName, "%s", GetName());
	char szMsg[128];
	//sprintf(szMsg, " %s[%u] %u.", pItem->szName, GridCont.sID, GridCont.sNum);
	sprintf(szMsg, RES_STRING(GM_CHARACTERCMD_CPP_00009), pItem->szName, GridCont.sID, GridCont.sNum);
	TL(CHA_SYS, szPlyName, "", szMsg);

	pCMap->ItemSpawn(&GridCont, lPosX, lPosY, enumITEM_APPE_THROW, pCCtrlCha->GetID(), pCMainCha->GetID(),
					 pCMainCha->GetHandle(), 10 * 1000); // 10
...
*/
	...
	*psThrowNum = GridCont.sNum;
	...
	if (IsBoat())
		...
		sprintf(szPlyName, "%s", GetName());
	char szMsg[128];
	//sprintf(szMsg, " %s[%u] %u.", pItem->szName, sItemID, GridCont.sNum);
...
Short CCharacter::Cmd_GuildBankOper(Char chSrcType, Short sSrcGridID, Short sSrcNum, Char chTarType, Short sTarGridID) {
	...
	if (chSrcType == chTarType) {
		...
		if (chSrcType == 0 && chTarType == 1) {
			...
			if (g_CParser.DoString("OnBankItem", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1,
								   pCMainCha, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, pGridCont, DOSTRING_PARAM_END)) {
...
Short CCharacter::Cmd_BankOper(Char chSrcType, Short sSrcGridID, Short sSrcNum, Char chTarType, Short sTarGridID) {
	...
	if (pCSrcBag == pCTarBag) //
	{
		...
		if (IsBoat())
			...
			sprintf(szPlyName, "%s", GetName());
		char szMsg[128];
		//sprintf(szMsg, "[%s-->%s] %s[%u] %u.", chSrcType == 0 ? "" : "", chTarType == 0 ? "" : "", pItem->szName, sSrcItemID, sLeftNum);
		sprintf(szMsg, RES_STRING(GM_CHARACTERCMD_CPP_00012),
				chSrcType == 0 ? RES_STRING(GM_CHARACTERCMD_CPP_00013) : RES_STRING(GM_CHARACTERCMD_CPP_00014),
				chTarType == 0 ? RES_STRING(GM_CHARACTERCMD_CPP_00013) : RES_STRING(GM_CHARACTERCMD_CPP_00014),
				pItem->szName, sSrcItemID, sLeftNum);
...
// bForcible
Short CCharacter::Cmd_RemoveItem(Long lItemID, Long lItemNum, Char chFromType, Short sFromID, Char chToType,
								 Short sToID, bool bRefresh, bool bForcible) {
	if (!bForcible) {
		//	2008-9-8	yangyinyu	add	begin!
		//		<>
		if (GetPlyMainCha()->m_CKitbag.IsLock())
...
void CCharacter::Cmd_FightAsk(dbc::Char chType, dbc::Long lTarID, dbc::Long lTarHandle) {
	...
	if (g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 2, this,
						   pCTeamFightEntry, DOSTRING_PARAM_END)) {
		if (!g_CParser.GetReturnNumber(0))
			return;
	}
	if (g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 2, pCTarCha,
						   pCTeamFightEntry, DOSTRING_PARAM_END)) {
		if (!g_CParser.GetReturnNumber(0)) {
			//SystemNotice("!");
	...
	if (g_CParser.DoString("Get_ItemAttr_Join", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this,
						   DOSTRING_PARAM_END))
		WRITE_SHORT(WtPk, g_CParser.GetReturnNumber(0));
	else
		WRITE_SHORT(WtPk, 0);
	if (g_CParser.DoString("Get_ItemAttr_Win", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, this,
						   DOSTRING_PARAM_END))
		WRITE_SHORT(WtPk, g_CParser.GetReturnNumber(0));
	else
		WRITE_SHORT(WtPk, 0);
	...
	if (chType == enumFIGHT_TEAM) {
		...
		while (pCTeamMem = pCPly->GetNextTeamPly()) {
			...
			if (!pCTeamCha->GetSubMap() || !(pCTeamCha->GetSubMap()->GetAreaAttr(pCTeamCha->GetPos()) &
				enumAREA_TYPE_FIGHT_ASK)) {
				//pCTeamCha->SystemNotice("!");
				pCTeamCha->SystemNotice(RES_STRING(GM_CHARACTERCMD_CPP_00029));
				continue;
			}
			if (g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 2,
								   pCTeamCha, pCTeamFightEntry, DOSTRING_PARAM_END)) {
				if (!g_CParser.GetReturnNumber(0))
					continue;
			}
			...
			if (g_CParser.DoString("Get_ItemAttr_Join", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1,
								   pCTeamCha, DOSTRING_PARAM_END))
				WRITE_SHORT(WtPk, g_CParser.GetReturnNumber(0));
			else
				WRITE_SHORT(WtPk, 0);
			if (g_CParser.DoString("Get_ItemAttr_Win", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1,
								   pCTeamCha, DOSTRING_PARAM_END))
				WRITE_SHORT(WtPk, g_CParser.GetReturnNumber(0));
			else
				WRITE_SHORT(WtPk, 0);
	...
	if (g_CParser.DoString("Get_ItemAttr_Join", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1,
						   pCTarCha, DOSTRING_PARAM_END))
		WRITE_SHORT(WtPk, g_CParser.GetReturnNumber(0));
	else
		WRITE_SHORT(WtPk, 0);
	if (g_CParser.DoString("Get_ItemAttr_Win", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1, pCTarCha,
						   DOSTRING_PARAM_END))
		WRITE_SHORT(WtPk, g_CParser.GetReturnNumber(0));
	else
		WRITE_SHORT(WtPk, 0);
...
void CCharacter::Cmd_FightAnswer(bool bFight) {
	...
	pCTarPly = g_pGameApp->IsValidPlayer(pCSrcPly->GetChallengeParam(2 + (chObjNum - 1) * 2),
										 pCSrcPly->GetChallengeParam(2 + (chObjNum - 1) * 2 + 1));
	...
	for (Char i = 0; i < chLoop; i++) {
		if (pCSrcPly->GetChallengeParam(i * 2 + 2) == pCPly->GetID() && pCSrcPly->GetChallengeParam(i * 2 + 2 + 1) ==
			pCPly->GetHandle())
			bHasPly = true;
	}
	if (!bHasPly) // PK
	...
	g_CParser.DoString(strScript1.c_str(), enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 3, pCMCpyCell,
					   pCSrcPly, pCTarPly, enumSCRIPT_PARAM_NUMBER, 1, lFightType, DOSTRING_PARAM_END);
	...
	pCSrcPly->ClearChallengeObj();
	for (Char i = 0; i < chLoop; i++) {
		pCFightMem = g_pGameApp->IsValidPlayer(pCSrcPly->GetChallengeParam(i * 2 + 2),
											   pCSrcPly->GetChallengeParam(i * 2 + 2 + 1));
		if (!pCFightMem)
			continue;
		...
		if (!pCFightCha->GetSubMap() || !(pCFightCha->GetSubMap()->GetAreaAttr(pCFightCha->GetPos()) &
			enumAREA_TYPE_FIGHT_ASK)) {
			//pCFightCha->SystemNotice("PKPK");
			pCFightCha->SystemNotice(RES_STRING(GM_CHARACTERCMD_CPP_00038));
			continue;
		}
		if (g_CParser.DoString(strScript2.c_str(), enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 2,
							   pCFightCha, pCTeamFightEntry, DOSTRING_PARAM_END)) {
			if (!g_CParser.GetReturnNumber(0))
				continue;
		}
		if (lFightType == enumFIGHT_TEAM) {
	...
	for (Long i = 0; i < lEnterChaNum; i++)
		g_CParser.DoString(strScript.c_str(), enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 2,
						   pCEnterCha[i], pCMCpyCell, DOSTRING_PARAM_END);
	...
	Char szPrint[10];
	_itoa(pCMCpyCell->GetPosID(), szPrint, 10);
	strPrint += szPrint;
	strPrint += ".";
	//strPrint += "";
...
void CCharacter::Cmd_ItemRepairAsk(dbc::Char chPosType, dbc::Char chPosID) {
	...
	if (g_CParser.StringIsFunction("can_repair_item")) {
		g_CParser.DoString("can_repair_item", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 3,
						   pCRepairman, this, pCPly->GetRepairItem(), DOSTRING_PARAM_END);
	...
	g_CParser.DoString("get_item_repair_money", enumSCRIPT_RETURN_NUMBER, 1, enumSCRIPT_PARAM_LIGHTUSERDATA, 1,
					   pCPly->GetRepairItem(), DOSTRING_PARAM_END);
...
void CCharacter::Cmd_ItemRepairAnswer(bool bRepair) {
	...
	if (bRepair) {
		...
		if (g_CParser.StringIsFunction("begin_repair_item")) {
			g_CParser.DoString("begin_repair_item", enumSCRIPT_RETURN_NONE, 0, enumSCRIPT_PARAM_LIGHTUSERDATA, 3,
							   pCRepairman, this, pCPly->GetRepairItem(), DOSTRING_PARAM_END);
			if (!pCPly->IsRepairEquipPos()) {
...

		{
			...
			for (int i = 1; i < defMAX_ITEM_LOTTERY_GROUP; i++) {
				...
				if (pItemGrid->sID == 5839)
					...
					_itoa(pItemGrid->sID - 5829, buffer[i - 1], 16);
			}
