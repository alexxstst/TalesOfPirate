...
void CGameApp::ProcessNetMsg(int nMsgType, GateServer* pGate, RPACKET pkt) {
	...
	{
		ToLogService("Connect", "Exec OnGateConnected()");
...
void CGameApp::OnGateConnected(GateServer* pGate, RPACKET pkt) {
	...
	ToLogService("Connect", "[{}]", g_pGameApp->m_strMapNameList.c_str());
...
void CGameApp::ProcessPacket(GateServer* pGate, RPACKET pkt) {
	...
	case CMD_TM_LOGIN_ACK: {
		...
		if (sErrCode = READ_SHORT(pkt)) {
			...
			ToLogService("GameLogin", "enter GateServer: {}:{} failed [{}], register map[{}]",
						 pGate->GetIP().c_str(), pGate->GetPort(), g_GameGateConnError(sErrCode),
						 g_pGameApp->m_strMapNameList.c_str());
			...
			if (!strcmp(pGate->GetName().c_str(), "")) {
				...
				/*LG("GameLogin", "?? GateServer: %s [%s:%d]??? [MapName:%s]\n",
					pGate->GetName().c_str(), pGate->GetIP().c_str(), pGate->GetPort(),
					g_pGameApp->m_strMapNameList.c_str());*/
				ToLogService("GameLogin", "entry GateServer: {} [{}:{}]success [MapName:{}]",
							 pGate->GetName().c_str(), pGate->GetIP().c_str(), pGate->GetPort(),
							 g_pGameApp->m_strMapNameList.c_str());
			}
		}
	...
	case CMD_TM_ENTERMAP: {
		...
		ToLogService("enter_map", "start entry map atorID = {} enter--------------------------", l_dbid);
		...
		if (!l_player) {
			...
			ToLogService("enter_map", "when create new palyer ID = {} assign memory failed ", l_dbid);
			return;
		}
		...
		ToLogService("enter_map", "end up entry map  [{}]================", pCCha->GetLogName());
		break;
	}
	case CMD_TM_GOOUTMAP: {
		...
		try {
			if (l_player->GetGateAddr() != l_gateaddr) {
				ToLogService("error", "DB ID: {}, address not matching??local :{:x}, gate:{:x},cmd={}, validity({}).",
							 l_player->GetDBChaId(), l_player->GetGateAddr(), l_gateaddr, cmd, l_player->IsValidFlag());
				break;
			}
		}
		catch (...) {
			ToLogService("error", "===========================from Gate plyaer's address error {},cmd ={}",
						 (void*)l_player, cmd);
			break;
		}
		if (!l_player->IsValid()) {
			ToLogService("enter_map", "this palyer already impotence");
			break;
		}
		if (l_player->GetMainCha()->GetPlayer() != l_player) {
			ToLogService(
				"error", "two player not matching??character name??{}??Gate address [local {}, guest {}]????cmd={}",
				l_player->GetMainCha()->GetLogName(), static_cast<void*>(l_player->GetMainCha()->GetPlayer()),
				static_cast<void*>(l_player), cmd);
		}

		ToLogService("enter_map", "start leave map--------");

		char chOffLine = READ_CHAR(pkt); // ???????(0)
		ToLogService("enter_map", "Delete Player [{}]", l_player->GetMainCha()->GetLogName());

		char szLogName[512];
		strncpy(szLogName, l_player->GetMainCha()->GetLogName(), 512 - 1);
		GoOutGame(l_player, !chOffLine, false);
		ToLogService("enter_map", "end and leave the map========");
			...
			if (cmd / 500 == CMD_PM_BASE / 500 && !l_player) {
				...
				try {
					...
					if (l_player->GetGateAddr() != l_gateaddr) {
						...
						ToLogService(
							"error", "DB ID:{}, address not matching??local :{}, gate:{},cmd={}, validity ({})",
							l_player->GetDBChaId(), l_player->GetGateAddr(),
							l_gateaddr, cmd, l_player->IsValidFlag());
						break;
					}
					...
					ToLogService(
						"error", "===========================Player address error that come from Gate {},cmd ={}",
						static_cast<void*>(l_player), cmd);
					break;
				}
				if (!l_player->IsValid())
				...
				if (g_pGameApp->IsValidEntity(pCCha->GetID(), pCCha->GetHandle())) {
					...
					ToLogService("error", "when receive CMD_CM_BASE message[{}], find character pCCha is null", cmd);
				}
				break;
			}
		}
...
void CGameApp::ProcessGuildChallMoney(GateServer* pGate, RPACKET pkt) {
	...
	if (pPlayer) {
		...
		ToLogService("challenge consortia result",
					 "not find deacon information finger??cannot back gold DBID[{}],how much money[{}].", dwChaDBID,
					 dwMoney);
	}
}
...
void CGameApp::ProcessGuildChallPrizeMoney(GateServer* pGate, RPACKET pkt) {
	...
	if (pPlayer) {
		...
		ToLogService("challenge consortia result",
					 "cannot find deacon information finger??cannot hortation DBID[{}],how much money[{}]", dwChaDBID,
					 dwMoney);
	}
}
...
void CGameApp::ProcessTeamMsg(GateServer* pGate, RPACKET pkt) {
	...
	for (char i = 0; i < cMemberCnt; i++) {
		...
		if (!Team[i].pGate) {
			ToLogService("team", "GameServer can't find matched Gate??{}, addr = 0x{:X}, chaid = {}.", pszGateName,
						 dwGateAddr, dwChaDBID);
			ToLogService("team", "\tGameServer all Gate:");
			BEGINGETGATE();
			GateServer* pGateServer;
			while (pGateServer = GETNEXTGATE()) {
				ToLogService("team", "\t{}", pGateServer->GetName().c_str());
			}
		}
