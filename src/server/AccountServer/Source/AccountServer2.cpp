...
void AccountServer2::Gs_Logout(DataSocket* datasock)
{
	...
	,()
	Gs_LoginGs_Logout,,
	GroupServer,AccountServer,,GroupServerAccountServer
	*/
...
void AuthThread::QueryAccount(RPacket rpkt)
{
	...
	m_AcctInfo.pDat = rpkt.ReadString(); // 
	...
	// SQL
	sprintf(szSql,
	        "select id, password, sid, login_status, login_group, ban, datediff(s, enable_login_time, getdate()) as protect_time from account_login where name='%s'",
	        m_AcctInfo.strName.c_str());
	rs << szSql; //account_loginname
...
WPacket AuthThread::AccountLogin(DataSocket* datasock)
{
	...
	wpkt.WriteCmd(0); //added by andor.zhang,GroupServerCMD_AP_KICKUSER
	...
	{
		sprintf(lpszSQL,
		        "update account_login set login_status=%d, login_group='%s', enable_login_time=getdate(), last_login_time=getdate(), last_login_mac='%s', last_login_ip='%s' where id=%d",
		        ACCOUNT_ONLINE, pFromGroupServer->GetName(), m_AcctInfo.strMAC.c_str(), m_AcctInfo.strIP.c_str(),
		        m_AcctInfo.nId);
	...
	else if (m_AcctInfo.nStatus == ACCOUNT_ONLINE)
	{
		//()
		sprintf(lpszSQL,
		        "update account_login set login_status=%d, login_group='%s', last_login_time=getdate(), last_login_mac='%s', last_login_ip='%s' where id=%d",
		        ACCOUNT_SAVING, pFromGroupServer->GetName(), m_AcctInfo.strMAC.c_str(), m_AcctInfo.strIP.c_str(),
		        m_AcctInfo.nId);
		if (m_pAuth->ExecuteSQL(lpszSQL))
		{
...
WPacket AuthThread::AccountLogin(DataSocket* datasock)
{
	...
	if (m_AcctInfo.bExist)
	{
		...
		if (g_TomService.IsEnable()) //Tom
		{
			...
			//if (eUserState==CTomService::eState_AllowLogin)				//
			//{
			//}
			//else if (eUserState==CTomService::eState_LoginLocked)		//
			//{
			//}
		}
		else //
		{
			bVerify = ChapSvr.DoAuth(m_AcctInfo.strName.c_str(), m_AcctInfo.strChapString.c_str(), m_AcctInfo.pDat,
			                         m_AcctInfo.usDatLen, m_AcctInfo.strPwdDigest.c_str());
			ChapSvr.NewSessionKey();
			if (bVerify)
			{
				// Sid
				...
				sprintf(szSql, szFmt,
				        ACCOUNT_OFFLINE, ACCOUNT_ONLINE, // 
				        ACCOUNT_ONLINE, ACCOUNT_SAVING, // 
				        //ACCOUNT_SAVING, ACCOUNT_ONLINE, //     2006-2-24 arcol_test
				        ACCOUNT_SAVING, ACCOUNT_SAVING, //     2006-2-24 arcol_test 

				        ACCOUNT_OFFLINE, RELOGIN_TIME,
				        ACCOUNT_ONLINE, SAVING_TIME,
				        //ACCOUNT_SAVING, RELOGIN_TIME,    2006-2-24 arcol_test
				        ACCOUNT_SAVING, SAVING_TIME, // 2006-2-24 arcol_test 
				...
				*/
				...
				bool bIsFromSameGroup = pGs->IsSame(m_AcctInfo.strGroup);
				//Arcol 06-2-14 BUG
				string strLastLoginGroup = m_AcctInfo.strGroup;
				//Arcol 06-2-14 BUG
				...
				if (nLinesAffected == 1)
				{
					if (m_AcctInfo.nStatus == ACCOUNT_ONLINE)
					{
						// 
						sprintf(szSql,
						        "update account_login set last_login_time=getdate(), last_login_mac = '%s' where id=%d",
					...
					else if (m_AcctInfo.nStatus == ACCOUNT_SAVING)
					{
						// 
						sprintf(szSql,
						        "update account_login set last_logout_time=getdate(), total_live_time=total_live_time+datediff(second, last_login_time, getdate()) where id=%d",
						        m_AcctInfo.nId);
						m_pAuth->ExecuteSQL(szSql);

						//if (pGs->IsSame(m_AcctInfo.strGroup))					//Arcol 06-2-14 BUG
						if (bIsFromSameGroup)
						{
							wpkt.WriteCmd(CMD_AP_RELOGIN);
							//wpkt.WriteShort(ERR_AP_ONLINE);					//
						...
						{
							// GroupServer
							//KickAccount(m_AcctInfo.strGroup, m_AcctInfo.nId);	//Arcol 06-2-14 BUG
...
void AuthThread::AccountLogout(RPacket rpkt)
{
	...
	sprintf(lpszSQL,
	        "update account_login set login_status=%d, login_group='', enable_login_time=getdate(), last_logout_time=getdate(), total_live_time=total_live_time+datediff(second, last_login_time, getdate()) where id=%d",
	        ACCOUNT_OFFLINE, nID); //   by jampe
	if (m_pAuth->ExecuteSQL(lpszSQL))
	{
		SetRunLabel(0);
	}
