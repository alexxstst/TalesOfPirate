...
public:
	DisableCloseButton(dbc::uLong interval) : dbc::Timer(interval), m_hMenu(nullptr) {
...
GateServer::GateServer(char const* fname)
	: client_heap(1, 2000), m_tch(1, 200), gm_conn(nullptr), gp_conn(nullptr), cli_conn(nullptr)
	...
	try {
		gm_conn = new ToGameServer(fname, nullptr, m_gmcomm);
...
void GateServer::RunLoop() {
	...
	while (!g_exit) {
		...
		if (l_str == "exit" || g_exit) {
			...
			//std::cout<<""<<cli_conn->GetSockTotal()<<std::endl;
			...
			std::cout << RES_STRING(GS_GATESERVER_CPP_00004) << l_band.m_sendpktps << "}{pkt:" << l_band.m_sendpkts <<
				"}{KB/s:" << l_band.m_sendbyteps / 1024 << "}{KB:" << l_band.m_sendbytes / 1024 << "}" << std::endl;
			...
			std::cout << RES_STRING(GS_GATESERVER_CPP_00005) << l_band.m_recvpktps << "}{pkt:" << l_band.m_recvpkts <<
				"}{KB/s:" << l_band.m_recvbyteps / 1024 << "}{KB:" << l_band.m_recvbytes / 1024 << "}" << std::endl;
			...
			std::cout << RES_STRING(GS_GATESERVER_CPP_00006) << sendpkps_max << "}{KB/s:" << sendbandps_max << "}" <<
				std::endl;
			//std::cout<<"[Max]{pkt/s:"<<recvpkps_max<<"}{KB/s:"<<recvbandps_max<<"}"<<std::endl;
			std::cout << RES_STRING(GS_GATESERVER_CPP_00007) << recvpkps_max << "}{KB/s:" << recvbandps_max << "}" <<
				std::endl;
		}
		else if (l_str == "clmax") {
			...
			//std::cout<<""<<g_gtsvr->cli_conn->GetMaxCon()<<std::endl;
			...
			if (l_maxcon > 1500) {
				...
				//std::cout<<":"<<l_maxcon<<std::endl;
				std::cout << RES_STRING(GS_GATESERVER_CPP_00010) << l_maxcon << std::endl;
			}
			g_gtsvr->cli_conn->SetMaxCon(l_maxcon);
		}
		else if (l_str == "getqueparm") {
			std::cout << "ToClient Process Queue:" << m_clproc->GetTaskCount() << "\tToClint Comm Queue:" << m_clcomm->
				GetTaskCount() << std::endl;
			std::cout << "ToGroup Comm Queue:" << m_gpcomm->GetTaskCount() << "\tToGame Comm Queue:" << m_gmcomm->
				GetTaskCount() << std::endl;
		}
...
bool ClientConnection::EncryptAES(char* ciphertext, dbc::uLong ciphertext_len, dbc::cChar* plaintext,
								  dbc::uLong& plainsize) {
	try {
		...
		CryptoPP::StringSource ss((const CryptoPP::byte*)plaintext, plainsize, true,
								  new CryptoPP::AuthenticatedEncryptionFilter(
									  e, new CryptoPP::StringSink(output), false, 12));
...
bool ClientConnection::DecryptAES(dbc::cChar* ciphertext, char* plaintext, dbc::uLong& ciphersize) {
	try {
		...
		CryptoPP::StringSource ss((CryptoPP::byte*)ciphertext, ciphersize, true,
								  new CryptoPP::Base64Decoder(new CryptoPP::StringSink(base64decoded)));
		CryptoPP::AuthenticatedDecryptionFilter df(d, new CryptoPP::StringSink(plain),
