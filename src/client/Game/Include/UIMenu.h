...
{
public:
	CMenuItem( const std::string& str, DWORD v );
	...
	virtual const std::string&	GetString()	{ return _strText;	}
...
public:
	...
	static CMenu*		FindMenu( const std::string& name );
	static int			CloseAll();

	void				SetAllEnabled( bool isEnabled );
	bool				IsAllDisabled();

	CMenuItem*			FindMenuItem( const std::string& name );
	...
	CMenuItem*			AddMenu( const std::string& szText );
...
inline CMenuItem* CMenu::FindMenuItem( const std::string& name )
{
