...
inline void CEffDelay::SetServerHarm(CHitRepresent& Hit, CServerHarm* pHarm) {
	if (_eStyle != enumNone) {
		ToLogService("CEffDelay", "msgCEffDelay::SetServerHarm() Style[{}] is Valid",
					 static_cast<std::int32_t>(_eStyle));
	}
...
inline void CEffDelay::SetPosEffect(int nEffID, const D3DXVECTOR3& pos) {
	if (_eStyle != enumNone) {
		ToLogService("CEffDelay", "msgCEffDelay::SetPosEffect(nEffID[{}],pos[{},{},{}]) Style[{}] is Valid", nEffID,
					 pos.x, pos.y, pos.z, static_cast<std::int32_t>(_eStyle));
	}
	_eStyle = enumPos;
