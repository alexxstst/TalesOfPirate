...
void CGameScene::_RecordRBO()
{
	...
	for( auto itr = _reallyBigObjectList.begin();
...
{ 
    ...
    ToLogService("scene init", "CGameScene::_Clear");
	return true;
}
...
#if 1
    auto it = _SceneObjIdx[SCENEOBJ_TYPE_POSE].begin();
    auto it_end = _SceneObjIdx[SCENEOBJ_TYPE_POSE].end();
...
{
    ToLogService("memory", "Create - Map {}, Eff: {}, Cha: {}, Obj: {}, Item: {}", _stInit.strMapFile.c_str(), _stInit.nMaxEff, _stInit.nMaxCha, _stInit.nMaxObj, _stInit.nMaxItem);
	ToLogService("scene init", "Creatememory {}", _stInit.strMapFile.c_str());
    _pMainCha = NULL;
	...
	ToLogService("scene init", "create array");
	...
	ToLogService("scene init", "CreateShadeArray");
	ToLogService("scene init", "init ok");
...
{
    ToLogService("memory", "\tClear - Map {}, Eff: {}, Cha: {}, Obj: {}, Item: {}", _stInit.strMapFile.c_str(), _stInit.nMaxEff, _stInit.nMaxCha, _stInit.nMaxObj, _stInit.nMaxItem);
		...
		if( nDistX*nDistX + nDistY*nDistY <= nDist )
		{
			for (j = nStartY; j <= nEndY; j++)
			{
				for (k = nStartX; k <= nEndX; k++)
				{
					...
					{
						ToLogService("pathfind", "msgCGameScene::AddCharacterBlock nRange[{}] isn't Valid, Cha name[{}], pos:[{}, {}]", nRange, pCha->getName(), pCha->GetCurX(), pCha->GetCurY());
						return 0;
					}
				}
			}
...
{    
	...
	if( !pMapInfo ) 
	{
		ToLogService("switchmap", "msgCGameScene::SwitchMap - GetMapInfo({}) return NULL", nMapID);
		return false;
	}
...
bool CGameScene::_Init()
{
    ...
    ToLogService("scene init", "3d cursor init");
	...
	CBoxMgr::CloseAllBox();
    ToLogService("scene init", "CGameScene::_Init");
	return true;
}

int CGameScene::UpdateSceneAnimLight()
{
    auto it = _SceneObjIdx[SCENEOBJ_TYPE_POINTLIGHT].begin();
    auto it_end = _SceneObjIdx[SCENEOBJ_TYPE_POINTLIGHT].end();
...
{
	ToLogService("scene init", "LoadingCall:{}\t{}", _stInit.strMapFile.c_str(), GetTickCount());
	...
	ToLogService("scene init", "create small map");
//////////////////////////////////////////////////////////////////////////
...
bool CGameScene::DelAreaEff( long nAreaID, int nEffectID )
{
    ...
    if( nEffectID==0 )
    {
        for( auto it=_areaeffs.begin(); it!=_areaeffs.end(); it++ )
        {
