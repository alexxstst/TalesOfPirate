	...
	if (_ModelType == MODEL_WARSHIP) {
		if (part_id == 0) {
			...
			for (DWORD i = 0; i < 4; i++) {
				...
				if (iiir == nullptr || iiir->chModule[0] == nullptr) {
...
int CCharacterModel::LoadPose(int cha_type) {
	...
	if (_TypeID == 1) {
		...
		if (anim_agent) {
			...
			MPIAnimCtrlObjBone* ctrl_obj_bone = static_cast<MPIAnimCtrlObjBone*>(anim_agent->GetAnimCtrlObj(&type_info));
			MPIAnimCtrlBone* ctrl_bone = static_cast<MPIAnimCtrlBone*>(ctrl_obj_bone->GetAnimCtrl());
...
DWORD CCharacterModel::_BigPose2SmallPose(DWORD pose_id) {
	...
	int iii = 0;
	CPoseInfo* info = ::GetPoseInfo(static_cast<short>(pose_id));
	if (info == nullptr) {
		ToLogService("pose_id", "{} {}", g_oLangRec.GetString(28), pose_id);
		animationLogger.LogWarning("PoseId: {} {}", pose_id, g_oLangRec.GetString(28));
		return -1;
	}
	...
	if (left_item) {
		...
		l = static_cast<BYTE>(ir->sType);
	}
	if (right_item) {
		ir = right_item->GetItemInfo();
		r = static_cast<BYTE>(ir->sType);
	}

	for (auto i : __wield_state) {
		if (i.Check(l, r) == 1) {
			iii = i.mode;
			break;
		}
...
void CCharacterModel::PlayPose(DWORD pose, DWORD type, int time, int fps, DWORD blend_flag, DWORD blend_src_num,
	...
	if (pi == nullptr) {
		animationLogger.LogError(
			"msgCCharacterModel::PlayPose GetPoseInfo() error with type: {}, pose:{}, SmallPoseID:{}",
			_TypeID, pose, _SmallPoseID);
		return;
	}
	if (pi->end - pi->start <= 0) {
		animationLogger.LogWarning("pose msgpose[{}]-pose[{}],[{}, {}]",
	...
	float velocity = 1.0f;
	if (time > 0) {
		float v = 1000.0f / static_cast<float>(fps) / (static_cast<float>(time) / static_cast<float>(pi->end - pi->start));
		velocity = v;
	}
	else {
		float v = 30.0f / static_cast<float>(fps);
		velocity = v;
	}
...
int CCharacterModel::AttachItem(DWORD link_id, CSceneItem* obj, DWORD part_type, DWORD item_link_id) {
	...
	if (AttachItem(link_id, &info) == 0) {
		ToLogService("error", "msgType: {}, link_id: {}, item_id: {}", obj->GetItemInfo()->szName, link_id,
					 item_link_id);
		return 0;
	}
	return 1;
...
CSceneItem* CCharacterModel::DetachItem(CSceneItem* obj) {
	CSceneItem* ret_obj = nullptr;

	for (auto& i : _LinkItem) {
		if (i && i == obj) {
			ret_obj = i;
			i->GetObject()->ClearLinkCtrl();
			// added by clp
			i->setParentCharacter(nullptr);
			i->unbindEffect();
...
int CCharacterModel::IsPosePlaying(DWORD pose) {
	if (_ModelType == MODEL_CHARACTER) {
		...
		if (info == nullptr || info->pose != pose)
			return 0;
...
void CCharacterModel::SetOpaque(float opaque) {
	if (opaque < 0.0f || opaque > 1.0f) {
		MessageBox(nullptr, "msgSetOpaque Error", "error", 0);
	}
