...
inline void SetPreName(int nItem, char* szName, DWORD& dwColor) {
	...
	switch (nItem) {
	case 3936: strcpy(szName, g_oLangRec.GetString(7));
		dwColor = D3DCOLOR_ARGB(255, 192, 192, 192);
		return;
	case 3937: strcpy(szName, g_oLangRec.GetString(8));
		dwColor = D3DCOLOR_ARGB(255, 000, 255, 012);
		return;
	case 3938: strcpy(szName, g_oLangRec.GetString(9));
		dwColor = D3DCOLOR_ARGB(255, 217, 255, 119);
		return;
	case 3939: strcpy(szName, g_oLangRec.GetString(10));
		dwColor = D3DCOLOR_ARGB(255, 070, 182, 240);
		return;
	case 3940: strcpy(szName, g_oLangRec.GetString(11));
		dwColor = D3DCOLOR_ARGB(255, 255, 207, 000);
		return;
	case 3941: strcpy(szName, g_oLangRec.GetString(12));
		dwColor = D3DCOLOR_ARGB(255, 255, 000, 000);
		return;
	case 3942: strcpy(szName, g_oLangRec.GetString(13));
		dwColor = D3DCOLOR_ARGB(255, 241, 014, 240);
		return;

	case 5331: strcpy(szName, g_oLangRec.GetString(922));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,192,192,192); return;
	case 5332: strcpy(szName, g_oLangRec.GetString(923));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,135,135,135); return;
	case 5333: strcpy(szName, g_oLangRec.GetString(924));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,255,255,255); return;
	case 5334: strcpy(szName, g_oLangRec.GetString(925));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,070,182,240); return;
	case 5335: strcpy(szName, g_oLangRec.GetString(926));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,000,255,012); return;
	case 5336: strcpy(szName, g_oLangRec.GetString(927));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,217,255,119); return;
	case 5337: strcpy(szName, g_oLangRec.GetString(928));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,255,207,000); return;
	case 5338: strcpy(szName, g_oLangRec.GetString(929));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,255,127,000); return;
	case 5339: strcpy(szName, g_oLangRec.GetString(930));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,255,000,000); return;
	case 5340: strcpy(szName, g_oLangRec.GetString(931));
		dwColor = COLOR_SKYBLUE;
		return; //D3DCOLOR_ARGB(255,241,014,240); return;

	// TOM
	case 822: strcpy(szName, g_oLangRec.GetString(14));
		dwColor = D3DCOLOR_ARGB(255, 255, 000, 000);
		return; //
	case 823: strcpy(szName, g_oLangRec.GetString(15));
		dwColor = D3DCOLOR_ARGB(255, 241, 014, 240);
		return; //
	default: szName[0] = '\0';
	}
...
int CCharacter::LoadCha(const LoadChaInfo* info) {
	...
	if (info->bone[0] != 0) {
		if (FAILED(LoadBone( info->bone ))) {
			ToLogService("error", "{}", g_oLangRec.GetString(16));
			return FALSE;
		}
	}
	...
	for (DWORD i = 0; i < 4; i++) {
		...
		if (p == NULL) {
			ToLogService("error", "msgLoad Ship Power System error!");
			return FALSE;
		}
...
void CCharacter::setPos(int nX, int nY) {
	...
	if (IsMainCha()) {
		...
		if (dis > 300.0f)
			ToLogService("main_cha_pos", "\t\tpos:{}, {}, target:{}, {}, dis:{}, Tick:{}, FPS: {}", _nCurX, _nCurY, nX,
						 nY, dis, GetTickCount() - tick, g_Render.GetFPS());
		else
			ToLogService("main_cha_pos", "pos:{}, {}, target:{}, {}, dis:{}, Tick:{}, FPS: {}", _nCurX, _nCurY, nX, nY,
...
void CCharacter::setSideID(long v) {
	...
	if (_lSideID != 0 && IsPlayer()) {
		ToLogService(getLogName(), "Add SideID:{}", _lSideID);
		_pSideShade = _pScene->GetFirstInvalidShadeObj();
		if (_pSideShade) {
			ToLogService(getLogName(), "Add SideID:{} Successed", _lSideID);
...
bool CCharacter::PlayPose(DWORD pose, DWORD type, int time, int fps, bool isBlend, bool IsGlitched) {
	...
	ToLogService(getLogName(), "Pose:{}, type:{}, time:{}", pose, type, time);
	animationLogger.LogWarning("Pose:{}, type:{}, time:{}", pose, type, time);
	...
	if (GetIsFly()) {
		...
		if (!itemRec) {
			animationLogger.LogError("Failed to get item record for mount ID: {}", mountID);
			return false;
		}
...
xShipInfo* CCharacter::ConvertPartTo8DWORD(stNetChangeChaPart& stPart, DWORD* dwBuf) {
	...
	if (!pInfo) {
		ToLogService("boat_error", "{} {}", g_oLangRec.GetString(20), stPart.sBoatID);
		return NULL;
	}
	...
	if (pData == NULL) {
		ToLogService("boat_error", "{} {}", g_oLangRec.GetString(21), pInfo->sBody);
		return NULL;
	}
	...
	if (pInfo->byIsUpdate) {
		...
		if (pData == NULL) {
			ToLogService("boat_error", "{} {}", g_oLangRec.GetString(22), stPart.sHeader);
			return NULL;
		}
		...
		if (pData == NULL) {
			ToLogService("boat_error", "{} {}", g_oLangRec.GetString(23), stPart.sEngine);
			return NULL;
		}
...
bool CCharacter::UpdataItem(int nItem, DWORD nLink) {
	...
	case enumEQUIP_SHOES:
		...
		if (CCharacterModel::ChangePart(nLink, nItem) == 0) {
			ToLogService(getLogName(), "msgChangePart Error, Item[{}], Link[{}]", nItem, nLink);
			return false;
		}
		return true;
		...
		if (nItem > 0 && nItem != enumEQUIP_BOTH_HAND) {
			...
			if (!_pHandItem[nLink] || _pHandItem[nLink]->GetItemInfo()->lID != nItem) {
				...
				if (item) {
					...
					ToLogService(getLogName(), "msgChangePart AddSceneItem Error, Item[{}], Link[{}]", nItem, nLink);
					return false;
				}
			}
		}
