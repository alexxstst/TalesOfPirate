...
CItemCommand::CItemCommand(CItemRecord* pItem)
	: _pItem(pItem), _pAniClock(nullptr), _pSkill(nullptr),
	  _dwPlayTime(0), _canDrag(true), _dwColor(COLOR_WHITE), _pBoatHint(nullptr) {
	if (!_pItem)
		ToLogService("error", "msgCItemCommand::CItemCommand(CItemRecord* pItem) pItem is NULL");
...
CItemCommand::CItemCommand(const CItemCommand& rhs)
	: _pImage(new CGuiPic(*rhs._pImage)), _pBoatHint(nullptr),
	  _pAniClock(nullptr), _dwPlayTime(0) {
	_Copy(rhs);
}
...
void CItemCommand::Render(int x, int y) {
	if ((_ItemData.expiration - std::time(nullptr)) <= 0 && _ItemData.expiration != 0 && _ItemData.sID != 32767) {
...
void CItemCommand::AddHint(int x, int y) {
	...
	if (isWeapon || isDefenceType || isJewelery || isEquip) {
		...
		if (_pItem->lID == 1034) {
			sprintf(buf, g_oLangRec.GetString(862), _ItemData.sEndure[0] * 10 - 1000,
		...
		if (isWeapon) {
			sprintf(buf, g_oLangRec.GetString(625), _GetValue(ITEMATTR_VAL_MNATK, item),
		...
		if (_ItemData.sNeedLv)
			PUSH_HINT(g_oLangRec.GetString(628), _ItemData.sNeedLv,
					  pAttr->get(ATTR_LV) >= _ItemData.sNeedLv ? GENERIC_COLOR : VALID_COLOR);
		else
			PUSH_HINT(g_oLangRec.GetString(628), _pItem->sNeedLv,
	...
	else if (_pItem->sType == 46) //
	{
		...
		string show_text[] = {
			g_oLangRec.GetString(650), g_oLangRec.GetString(651), g_oLangRec.GetString(652), g_oLangRec.GetString(653),
			g_oLangRec.GetString(654)
		};
		int value = 0;
	...
	else if (_pItem->sType == 71 && _pItem->lID == 3289) //
	{
		...
		const char* arShowName[5] = {
			g_oLangRec.GetString(944), g_oLangRec.GetString(945), g_oLangRec.GetString(946), g_oLangRec.GetString(947),
			g_oLangRec.GetString(948)
		}; // "", "", "", "", "" };
	...
	if (_hints.GetCount() > 0 && Forge.IsForge) {
		...
		for (int i = 0; i < Forge.nStoneNum && i < Forge.nHoleNum; i++) {
			sprintf(buf, g_oLangRec.GetString(656), ConvertNumToChinese(Forge.nStoneLevel[i]).c_str(),
	...
	if (pOwn) {
		...
		else if (GetItemInfo()->sType == 71) //
		{
			...
			if (nCoolDownTime > 0) //  cooldown
			{
				...
				auto it = _mapCoolDown.find(nSkillID);

				if (it != _mapCoolDown.end() && it->second + nCoolDownTime >= nCurTickCount) {
					// cooldown
					g_pGameApp->SysInfo(g_oLangRec.GetString(898),
										(it->second + nCoolDownTime - nCurTickCount) / 1000 + 1); //" %d "
					return false;
				}
...
void CItemCommand::_ShowBody(CItemRecord* _pItem2) {
	...
	for (int i = 1; i < 5; i++) {
		...
		switch (i) {
		case 1: str << g_oLangRec.GetString(681);
			break;
		case 2: str << g_oLangRec.GetString(682);
			break;
		case 3: str << g_oLangRec.GetString(683);
			break;
		case 4: str << g_oLangRec.GetString(684);
			break;
		}
	}
	str << '\0';
