...
bool CTreeNodeObj::ClearAllChild()
{
	...
	for( auto it=_ndChilds.begin(); it!=_ndChilds.end(); ++it )
	{
		//delete *it;
		SAFE_DELETE(*it); // UI
	}
...
void CTreeNodeObj::Render()
{
	...
	{
		auto end = GetChild().end();
		for( auto it=GetChild().begin(); it!=end; ++it )
		{
			(*it)->Render();
		}
	}
...
bool CTreeNodeObj::IsShow() const {
...
CTreeNodeObj* CTreeNodeObj::FindNode( const std::string& str )
{
    if( str == GetCaption())
    {
        return this;
    ...
    if( GetChild().empty() ) return nullptr;

    const auto end = GetChild().end();
    for( auto it=GetChild().begin(); it!=end; ++it )
    {
        if(str == (*it)->GetCaption())
            return *it;
    }
...
bool CTreeGridNode::DelItem( const std::string& str )
{
    for( auto it=_items.begin(); it!=_items.end(); ++it )
    {
        if( str == (*it)->GetString() )
