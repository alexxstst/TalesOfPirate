	...
	*/
	...
	if (strParam.find("pack") != std::string::npos) {
		...
		if (pkfile.SaveToPack(pszpath, pszFile))
			ToLogService("ok", "{} {}", g_oLangRec.GetString(188), pszPos);
		else
			ToLogService("ok", "{} {}", g_oLangRec.GetString(189), pszPos);
		return 0;
	}
	...
	if (!g_pGameApp->Init(hInstance, szWindowClass, nWidth, nHeight, nDepth, FALSE /*g_Config.m_bFullScreen*/)) {
		ToLogService("init", "{}", g_oLangRec.GetString(191));
		g_pGameApp->End();
		return 0;
	}
	...
	ToLogService("init", "init use time = {} ms", t.End());
