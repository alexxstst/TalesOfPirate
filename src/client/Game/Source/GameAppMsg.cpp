...
long CALLBACK TerrainNotice(int nFlag, int nSectionX, int nSectionY, unsigned long dwParam, MPTerrain* pThis)
{
	...
	// bug//by clp
	auto itr = g_pGameApp->GetCurScene()->Begin_RBO();
	auto end = g_pGameApp->GetCurScene()->End_RBO();
	while ( itr != end )
	{
...
void CGameApp::HandleKeyDown(DWORD dwKey)
{
	...
	// 
    // , HandleSuperKey()
    if(IsKeyDown(DIK_F1) && IsCtrlPress() ) // |SuperKey
    {
