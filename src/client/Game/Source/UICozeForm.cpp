...
void CCharMsg::AddMsg(eChannel channel, string strWho, string strText, bool bSendTo,DWORD dwcolour)
{
	...
	{
		//
		auto iterRemoveObject=m_sChannelInfo[wChannelIndex].itFirstMsgPos;
		...
		{
			for (auto it=m_lstThisInstanceLink.begin(); it!=m_lstThisInstanceLink.end(); it++)
			{
...
bool CCharMsg::MoveToNextMsg()
{
	...
	auto iterTemp=m_itCurrentMsgPos;
...
void CCardCase::AddCard(string str)
{
	...
	for (auto it=m_lstCardData.begin();it!=m_lstCardData.end();it++)
	{
		if ((*it)==str)
		{
...
{
	auto iterTemp=m_itCurrentCardPos;
...
{
	FORM_LOADING_CHECK(m_frmChannelSwitch,"main.lua","frmForbid");
	m_frmChannelSwitch->evtLost=EventLostFocus;
	FORM_CONTROL_LOADING_CHECK(m_chkPrivate,m_frmChannelSwitch,CCheckBox,"main.lua","chkPersonal");
	FORM_CONTROL_LOADING_CHECK(m_chkSight,m_frmChannelSwitch,CCheckBox,"main.lua","chkNear");
	FORM_CONTROL_LOADING_CHECK(m_chkSystem,m_frmChannelSwitch,CCheckBox,"main.lua","chkSystem");
	FORM_CONTROL_LOADING_CHECK(m_chkTeam,m_frmChannelSwitch,CCheckBox,"main.lua","chkTeam");
	FORM_CONTROL_LOADING_CHECK(m_chkGuild,m_frmChannelSwitch,CCheckBox,"main.lua","chkGuild");
	FORM_CONTROL_LOADING_CHECK(m_chkWorld,m_frmChannelSwitch,CCheckBox,"main.lua","chkWorld");
	FORM_CONTROL_LOADING_CHECK(m_chkTrade,m_frmChannelSwitch,CCheckBox,"main.lua","chkTrade");
...
void CCozeForm::OnSightMsg(CCharacter* pChar, string strMsg,DWORD dwColour)
{
	...
	string str = string(pChar->IsPlayer() ? pChar->getHumanName() : pChar->getName())+": "+strMsg;// ? ning.yan 2008-11-07
...
bool CCozeForm::Init()
{
	...
	FORM_LOADING_CHECK(m_frmPublish, "main.lua", "frmGM");
	m_frmPublish->evtBeforeShow=EventPublishShowForm;

	FORM_CONTROL_LOADING_CHECK(m_edtPublishMsg, m_frmPublish, CEdit, "coze.lua", "edtGMSay");
	m_edtPublishMsg->SetIsWrap(true);
	m_edtPublishMsg->evtEnter=EventPublishSendMsg;

	FORM_CONTROL_LOADING_CHECK(m_edtMsg,m_frmMainChat,CEdit,"main.lua","edtSay");
	m_edtMsg->SetIsWrap(true);
	m_edtMsg->SetIsParseText(true);
	m_edtMsg->evtEnter=EventSendMsg;
	m_edtMsg->evtKeyDown=EventEditMsg;

	FORM_CONTROL_LOADING_CHECK(m_cmbChannel,m_frmMainChat,CCombo,"main.lua","cboChannel");
	m_cmbChannel->evtSelectChange=EventSendChannelChange;
	m_cmbChannel->evtMouseClick=EventSendChannelSwitchClick;
	m_cmbChannel->GetList()->GetItems()->Select(0); 

	FORM_CONTROL_LOADING_CHECK(m_lstMainPage,m_frmMainChat,CList,"main.lua","lstOnSay");
	...
	FORM_CONTROL_LOADING_CHECK(m_lstSystemPage,m_frmMainChat,CList,"main.lua","lstOnSaySystem");
	...
	FORM_CONTROL_LOADING_CHECK(m_drgMainPage,m_frmMainChat,CDragTitle,"main.lua","drpTitle");
	...
	FORM_CONTROL_LOADING_CHECK(m_drgSystemPage,m_frmMainChat,CDragTitle,"main.lua","drpTitleSystem");
	...
	FORM_CONTROL_LOADING_CHECK(m_chkChannelSwitch,m_frmMainChat,CCheckBox,"main.lua","chkChatOn");
	m_chkChannelSwitch->SetIsChecked(false);
	m_chkChannelSwitch->evtCheckChange=EventChannelSwitchCheck;

	FORM_CONTROL_LOADING_CHECK(m_btnCallingCardSwitch,m_frmMainChat,CTextButton,"main.lua","btnCard");
	m_btnCallingCardSwitch->evtMouseClick=EventCallingCardSwitchClick;		//switch

	FORM_CONTROL_LOADING_CHECK(m_lstCallingCard,m_frmMainChat,CList,"main.lua","lstCard");
	...
	FORM_CONTROL_LOADING_CHECK(m_btnFaceSwitch,m_frmMainChat,CTextButton,"main.lua","btnChatFace");
	m_btnFaceSwitch->evtMouseClick=EventFacePanelSwitchClick;		//switch

	FORM_CONTROL_LOADING_CHECK(m_grdFacePanel,m_frmMainChat,CGrid,"main.lua","grdFace");
	//m_grdFacePanel->evtLost=EventFacePanelLostFocus;
	m_grdFacePanel->evtSelectChange=EventFaceSelected;

	FORM_CONTROL_LOADING_CHECK(m_btnBrowSwitch,m_frmMainChat,CTextButton,"main.lua","btnBrow");
	m_btnBrowSwitch->evtMouseClick=EventBrowPanelSwitchClick;		//switch

	FORM_CONTROL_LOADING_CHECK(m_grdBrowPanel,m_frmMainChat,CGrid,"main.lua","grdHeart");
	m_grdBrowPanel->evtSelectChange=EventBrowSelected;

	FORM_CONTROL_LOADING_CHECK(m_btnActionSwitch,m_frmMainChat,CTextButton,"main.lua","btnAction");
	m_btnActionSwitch->evtMouseClick=EventActionPanelSwitchClick;		//switch

	FORM_CONTROL_LOADING_CHECK(m_grdActionPanel,m_frmMainChat,CGrid,"main.lua","grdAction");
