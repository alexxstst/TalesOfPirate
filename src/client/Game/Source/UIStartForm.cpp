...
bool CStartMgr::Init()
{
 ...
 //  	proMainSP->SetPosition (10.0f );
	...
	//frmMainFun
	{
		...
		// QQ
		/*FORM_CONTROL_LOADING_CHECK(btnQQ,frmMainFun,CTextButton,"msgui.lua","btnQQ");
		...
		//FORM_CONTROL_LOADING_CHECK(btnLevelUpHelp, frmMainFun, CTextButton, "msgui.lua", "btnLevelUpHelp");
...
void CStartMgr::UpdateShipSailForm()
{
	...
	labLevelShow->SetCaption(_itoa(pAttr->get(ATTR_LV), buf, 10));
	labExpShow->SetCaption(_itoa(pAttr->get(ATTR_CEXP), buf, 10));
...
void  CStartMgr::PopMenu( CCharacter* pCha )
{
	...
	if ( mainMouseRight && pCha && pCha->IsValid() && !pCha->IsHide() && (pCha->IsPlayer() || pCha->IsMonster()) ) // && (pCha->IsPlayer() || pCha->IsMonster())
	{
		...
		for( int i=0; i<nCount; i++ )
		{
			...
			if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(764) )==0 )
			{
			...
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(482) )==0 )
			{
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled( pMain!=pCha );
			}
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(484) )==0 )
			...
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(483) )==0 )
			{	//   Add by ning.yan  20080715 Begin
				//if( _stricmp( MapName,"starena1") == 0 || _stricmp( MapName,"starena2") == 0 || _stricmp( MapName,"starena3") == 0 )
				//	pItem->SetIsEnabled( false );
				//// End
			...
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(481) )==0 )
			{
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled( pMain!=pCha );
			}
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(766) )==0 )
			{
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled( pMain!=pCha && pMain->IsBoat() && pMain->IsEnabled() && pCha->IsBoat() && pCha->IsEnabled() );
			}
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(768) )==0 )
			{
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled( pMain!=pCha && pMain->IsEnabled() && !pMain->IsShop() && pCha->IsEnabled() && pCha->IsShop() );
			}
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(769) )==0 )
			{
				pItem->SetIsHide(pCha->IsMonster());
			...
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(770) )==0 )
			{
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled( g_stUIStart.IsCanTeam() && pMain!=pCha && pCha->IsPlayer() );
			}
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(855) )==0 )	//
			{
				pItem->SetIsHide(pCha->IsMonster());
			...
			else if( _stricmp( pItem->GetString().c_str(), g_oLangRec.GetString(859) )==0 )	//
			{
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled( pMain!=pCha && pCha->IsPlayer() && pMain->getGameAttr() && pMain->getGameAttr()->get(ATTR_LV) > 40 );
							//&& pCha->getGameAttr()  && pCha->getGameAttr()->get(ATTR_LV) <= 40 );
			}else if( _stricmp( pItem->GetString().c_str(), "Check Eq" )==0 ){
			...
			else if( _stricmp( pItem->GetString().c_str(), "Follow") == 0){
				pItem->SetIsHide(pCha->IsMonster());
				pItem->SetIsEnabled(pMain!=pCha && pCha->IsPlayer());
			}
			else if( _stricmp( pItem->GetString().c_str(), "Block" )==0 )
			{
			...
			else if( _stricmp( pItem->GetString().c_str(), "Unblock" )==0 )
			{
				if (g_stUIChat.GetTeamMgr()->Find(enumTeamBlocked)->FindByName(pCha->getHumanName()) && !pCha->IsMonster())
				{
					pItem->SetIsHide( false );
