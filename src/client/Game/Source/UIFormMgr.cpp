...
bool CFormMgr::SetFormTempleteMax(int n)
{
	if( n>=0 ) 
	{
		for( auto it=_showforms.begin(); it!=_showforms.end(); ++it )
		{
			//delete *it;
			SAFE_DELETE(*it); // UI
		}
...
void CFormMgr::_InitFormID()
{
	...
	for( auto it=_allForms.begin(); it!=_allForms.end(); ++it )
	{
		(*it)->_TempleteClear();
	}
...
void CFormMgr::FrameMove( int x, int y, DWORD dwMouseKey, DWORD dwTime )
{
    ...
    if( dwTime>=dwNextTime )
    {
		...
		for( auto it=_modal.begin(); it!=_modal.end(); it++ )
		{
			(*it)->FrameMove( dwTime );
		}
...
bool CFormMgr::_MouseRun( int x, int y, DWORD mouse )
{
	...
	{
		for( auto it=_OnMouseRun.begin(); it!=_OnMouseRun.end(); ++it )
			(*it)( x, y, mouse );
	}
...
{
    auto it = find( _modal.begin(), _modal.end(), form );
    if( it!=_modal.end() )
    {
        _modal.erase( it );
	    _SetNewActiveForm();
...
void CFormMgr::_SetNewActiveForm()
{
	...
	else
	{
		...
		for( auto it=_show.begin(); it!=_show.end(); ++it )
		{
			if( (*it)->IsNormal() )
			{
				(*it)->_OnActive();
...
bool CFormMgr::MouseScroll( int nScroll )
{
	...
	{
		for( auto it=_OnMouseScroll.begin(); it!=_OnMouseScroll.end(); ++it )
			if( (*it)( nScroll ) )
				return true;
	}
...
bool CFormMgr::OnKeyDown(int key)
{
	...
	if( !_OnKeyDown.empty() )
	{
		const auto end=_OnKeyDown.end();
		for( auto it=_OnKeyDown.begin(); it!=end; ++it )
			if( (*it)( key ) )
				return true;
	}
...
bool CFormMgr::OnKeyChar(char key)
{
	...
	if( !_OnKeyChar.empty() )
	{
		const auto end=_OnKeyChar.end();
		for( auto it=_OnKeyChar.begin(); it!=end; ++it )
			if( (*it)( key ) ) 
				return true;
	}
...
bool CFormMgr::OnHotKey( char key, int control )
{
	...
	if( !_vhotkey.empty() )
	{
		for( auto it=_vhotkey.begin(); it!=_vhotkey.end(); ++it )
			if( (*it)(key, control) )
				return true;
	}
...
bool CFormMgr::HandleWindowMsg(DWORD dwMsg, DWORD dwParam1, DWORD dwParam2)
{
	...
	switch (dwMsg) 
	{
		case WM_HOTKEY:
		{
			...
			{
				const auto end = _forms->end();
				for( auto it=_forms->begin(); it!=end; ++it )
				{
...
{
	auto it = find( _OnKeyChar.begin(), _OnKeyChar.end(), event );
...
{
	auto it = find( _OnKeyDown.begin(), _OnKeyDown.end(), event );
...
{	
	auto it = find( _OnMouseRun.begin(), _OnMouseRun.end(), event );
...
{	
	auto it = find( _OnMouseScroll.begin(), _OnMouseScroll.end(), event );
...
{ 
	auto it = find( _vinits.begin(), _vinits.end(), pInitFun );
	if( _vinits.end() == it )
	{
...
void CFormMgr::_UpdataShowForm( CForm* frm )
{
    ...
    auto it = find( _show.begin(), _show.end(), frm );
    if( it!=_show.end() )
    {
        _show.erase( it );
        _show.push_back( frm );
...
void CFormMgr::ShowDebugInfo()
{
	...
	for( i=0;i<(int)_forms->size();i++ )
	{
		...
		DWORD color=COLOR_WHITE;
		//"ID                                                "
		sprintf(buf,"%-6d %-20s %s   %s     %s     %s     %3s    %3d    %2s     %4s",i,pInfo[i].formName.c_str(),pInfo[i].coordinate.c_str(),pInfo[i].isShow.c_str(),pInfo[i].isEnable.c_str(),pInfo[i].hasPopMenu.c_str(),(pInfo[i].layer>=0)?_itoa(pInfo[i].layer,temp,10):"N/A",
