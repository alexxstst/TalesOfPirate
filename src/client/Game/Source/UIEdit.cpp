...
void CEdit::SetCaption( const std::string& str)
{ 
	if( GetActive()==this )
	{
		if(str.size() <= this->GetMaxNum()) // Add by lark.li 20080820
		{
...
void CEdit::SetTextColor( std::uint32_t color )
{ 
	_color = color;		
}

void CEdit::ReplaceSel( const std::string& str, BOOL bCanUndo )
{
	if( GetActive()==this )
	{
