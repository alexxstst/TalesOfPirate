...
//---------------------------------------------------------------------------
#define ERR_CHK(p, s) { if(p == 0) { ToLogService("init", "ShipFactory Load error: {}", s); } }
...
BOOL xShipFactory::UpdateShipPart(CGuiData* sender)
{
    ...
    */
    ...
    sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(_itoa(prop_def, buf, 10));
    sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(_itoa(prop_speed, buf, 10));
    sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(_itoa(prop_cd, buf, 10));
    sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(_itoa(prop_dis, buf, 10));
...
BOOL xShipFactory::Update(xShipBuildInfo* info, BOOL flag, const char* name)
{
    ...
    sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(_itoa(info->dwDefence, buf, 10));
    sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(_itoa(info->dwSpeed, buf, 10));
    sbf.lbl_prop[SBF_TEXT_COOLDOWN]->SetCaption(_itoa(info->dwAttackTime, buf, 10));
    sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(_itoa(info->dwConsume, buf, 10));
	...
	sbf.lbl_prop[SBF_TEXT_MONEY]->SetCaption(_itoa(info->dwMoney, buf, 10));
...
void xShipFactory::SetBoatAttr()
{
	...
	sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(_itoa(m_pkBoatInfo->dwSpeed, buf, 10));
	sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(_itoa(m_pkBoatInfo->dwDistance, buf, 10));
	sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(_itoa(m_pkBoatInfo->dwDefence, buf, 10));
}
...
void xShipFactory::UpdateBoatAttr()
{
	...
	{ /**/
		...
		sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(_itoa(pkAttr->get(ATTR_BMSPD), buf, 10));
		sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(_itoa(pkAttr->get(ATTR_BDEF), buf, 10));
		sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(_itoa(pkAttr->get(ATTR_BSREC), buf, 10));
	}
	else
	{ /*()*/
		...
		sbf.lbl_prop[SBF_TEXT_SPEED]->SetCaption(_itoa(pkAttr->get(ATTR_MSPD), buf, 10));
		sbf.lbl_prop[SBF_TEXT_DEFENCE]->SetCaption(_itoa(pkAttr->get(ATTR_DEF), buf, 10));
		sbf.lbl_prop[SBF_TEXT_DISTANCE]->SetCaption(_itoa(pkAttr->get(ATTR_SREC), buf, 10));
	}
	...
	//
	sbf.lbl_ship_level->SetCaption(_itoa(pkAttr->get(ATTR_LV), buf, 10));
	sbf.lbl_ship_exp->SetCaption(_itoa(pkAttr->get(ATTR_CEXP), buf, 10));
