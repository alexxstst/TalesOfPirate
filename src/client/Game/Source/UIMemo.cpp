...
void CMemo::AddItemRowContent(int row ,const  char* szFunc, const char* szItemEx )
{
	if( !szFunc )
	{
		ToLogService("errormemo", "msgCMemo::AddItemRowContent szFunc is null!");
		return;
	}

	if( row>=MEMO_MAX_ITEM || row<0 )
	{
		ToLogService("errormemo", "msgCMemo::AddItemRowContent( {}, {} ) out of range!", row, szFunc);
		return;
	}
	//
	_strItem[row] = szFunc;
...
void CMemo::AddMisRowContent(int row ,const char *szFunc)
{
	if( !szFunc )
	{
		ToLogService("errormemo", "msgCMemo::AddMisRowContent szFunc is null!");
		return;
	}

	if( row>=MEMO_MAX_ITEM || row<0 )
	{
		ToLogService("errormemo", "msgCMemo::AddMisRowContent( {}, {} ) out of range!", row, szFunc);
		return;
	}
...
void CMemo::SetMisRowNum(int num ) 
{
	...
	{
		ToLogService("errormemo", "msgCMemo::SetMisRowNum( {} ) out of range!", num);
		if( num<0 ) num=0;
