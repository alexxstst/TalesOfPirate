...
void CTextParse::AddFace(std::uint32_t nIndex, CGraph* pGraph) {
	if (GetFace(nIndex)) return;

	if (nIndex < 0) ToLogService("error", "msgCTextParse::AddFace({}) error", nIndex);
...
CGraph* CTextParse::GetFace(std::uint32_t nIndex) {
