...
void NetPCTeam(stNetPCTeam  &pcteam)
{
	...
	ToLogService( "teamleader", "TeamID: {}", pcteam.cha_dbid[0] );
	char buffer[64];
	char buf[64];
	...
	ToLogService("Team", "Kind:[{}], LeaderID:{}", pcteam.kind, pcteam.cha_dbid[0] );
	...
	case TEAM_MSG_KICK:
        ...
        break;
    default: ToLogService("protocol", "msgNetPCTeam unknown kind:{}", pcteam.kind );
    }
...
{
	ToLogService("test_group", "{} {} {}", g_oLangRec.GetString(223), inviter_name, inviter_chaid);
	g_stTeamInviteFormMgr.AddInviteForm(inviter_chaid,inviter_name);
}

void NetTeamCancel(unsigned long inviter_chaid,char reason)
{
	ToLogService("test_group", "{} {} {}", g_oLangRec.GetString(224), inviter_chaid, reason);
	switch(reason)
	{
...
{
	ToLogService("test_frnd", "{} {} {}", g_oLangRec.GetString(230), inviter_name, inviter_chaid);
	g_stFrndInviteFormMgr.AddInviteForm(inviter_chaid,inviter_name);
}

void NetFrndCancel(unsigned long  inviter_chaid,char reason)
{
	ToLogService("test_frnd", "{} {} {}", g_oLangRec.GetString(231), inviter_chaid, reason);
	switch(reason)
	{
...
{
	ToLogService("test_frnd", "{} {}", g_oLangRec.GetString(236), cha_id);
...
{
	ToLogService("test_frnd", "{} {}", g_oLangRec.GetString(238), cha_id);
	CMember *pMember=g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Find(cha_id);
	if (pMember)
	{
...
{
	ToLogService("test_frnd", "{} {}", g_oLangRec.GetString(240), cha_id);
...
{//grp-
	ToLogService("test_frnd", "{} {} {}", g_oLangRec.GetString(241), cha_name, cha_id);
...
void NetFrndStart(stNetFrndStart &self,stNetFrndStart arrCha[],unsigned short arrnum)
{
	...
	for (int i=0;i<arrnum;i++)
	{
		ToLogService("test_frnd", "{} {} {} {}", g_oLangRec.GetString(242), i, arrCha[i].szChaname, arrCha[i].lChaid);
...
{
	ToLogService("test_frnd", "{} {} {} {} {} {}", g_oLangRec.GetString(243), cha_id, motto, icon, degree, job);
	CMember *pMember=g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Find(cha_id);
	if (pMember)
	{
...
extern void NetSessLeave(unsigned long sessid,unsigned long chaid)
{
	...
	if (g_stUIChat._dwSelfID!=chaid)
	{
		...
		{
			//char szBuf[2048] = { 0 };
			//sprintf( szBuf, "NetSessLeave(sessid:%d, chaid:%d, selfid:%d) error, pSession==NULL SessionCount=%d", sessid, chaid, g_stUIChat._dwSelfID, CTalkSessionFormMgr::GetSessionCount() );
