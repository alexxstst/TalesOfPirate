...
bool CForm::_AddFrameMove(CCompent* c, bool isCheck)
{
    if( isCheck )
    {
        auto it = find( _frames.begin(), _frames.end(), c );
        if( _frames.end() == it )
        {
            _frames.push_back(c);
			return true;
...
bool CForm::_AddCompent(CCompent* c, bool isCheck)
{
    if( isCheck )
    {
        auto it = find( _allCompents.begin(), _allCompents.end(), c );
        ...
        {
            auto it = find( _compents.begin(), _compents.end(), c );
            if( _compents.end() == it )
            {
...
{
	CCompent *p = NULL;
	for( auto it=rhs->_allCompents.begin(); it!=rhs->_allCompents.end(); it++ )
	{
...
void CForm::Init()
{
	...
	for( auto it =_compents.begin(); it!=_compents.end(); it++ )
	{
		(*it)->Init();
	}
...
{
	auto it =_mouse.begin();
	const auto end = _mouse.end();
...
{
    auto it =_compents.begin();
    const auto end = _compents.end();
    CCompent* pHint = NULL;
...
{
	for( auto it =_allCompents.begin(); it!=_allCompents.end(); it++ )
	{
