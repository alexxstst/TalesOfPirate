...
public:
	...
	struct MPFontRect
	{
		...
		bool IsEqual(const std::string& pszText, int x,int y, DWORD color,float tfscale)
		{
...
#ifdef		USE_RENDER
	bool CreateFont( MPRender* pd3dDevice, const std::string& szFontName, int nSize = 16, int nLevel = 3 ,DWORD dwFlag = 0);
#else
	...
	void Begin();
	void Draw(const std::string& szText, int x, int y, D3DXCOLOR color);
	void End();

	void BeginClip();
	void DrawTextClipOnce(const std::string& szText, int nLen, LPRECT psrc, LPRECT pclip,D3DXCOLOR color);
	void EndClip();

	bool DrawText( const std::string& szText, int x, int y, D3DXCOLOR color = 0xFFFFFFFF,float fScale = 1.0f ,DWORD* dwTime = NULL);
	bool DrawText( int iNumber, int x, int y, D3DXCOLOR color = 0xFFFFFFFF,float fScale = 1.0f );
	
	bool DrawTextShadow( const std::string& szText, int x1, int y1, int x2, int y2,
		D3DXCOLOR color1,D3DXCOLOR color2);


	bool Draw3DText(const std::string& szText,D3DXVECTOR3& vPos, D3DXCOLOR color = 0xFFFFFFFF,float fScale = 0.3f );
...
private:
	...
	void FillTextToTex( const std::string& szText );

	bool FindTextFromTex( char c1, char c2, float & tX1, float & tY1 , float & tX2, float & tY2);

	bool DrawTextClip( const std::string& szText, int nLen,LPRECT psrc, LPRECT pclip,D3DXCOLOR color = 0xFFFFFFFF);

	int FindRect( const std::string& szText, int x,int y, DWORD color,float tfscale)
	{
