...
void MPMap::Render()
{
       ...
       if(_pLandVB==NULL)
        {
	        ...
	        if(FAILED(hr)) 
	        {
                LG("error", "msg MPMap::Render() Land Terrain Vertex Buffer!\n");
                _pLandVB = NULL;
            }
        }
    }
    ...
    if(_bUseVB)
    {
        ...
        if(_pVB==NULL)
        {
	        ...
	        if(FAILED(hr)) 
	        {
                LG("error", "msg MPMap::Render() Sea Vertex Buffer!\n");
                _pVB = NULL;
            }
        }
#endif
...
BOOL MPMap::_AddRenderGroup(BYTE btLayer, int nTexNo, MPTile *pTile, short sTileX, short sTileY)
{
   ...
   {
		LG("error", "msgAddRenderGroup%d", MAX_RENDER_GROUP);
		return FALSE;
   }
   if(btLayer>=4)
   {
		LG("error", "msgAddRenderGroup%d", 3);
		return FALSE;
   }
...
groupend:
    ...
    if(bFull) LG("error", "msg(>%d)", MAX_RENDER_TILE * 2);
