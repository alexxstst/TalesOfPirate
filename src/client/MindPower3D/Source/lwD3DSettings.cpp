...
LW_RESULT lwSaveD3DSettings( const char* file, const lwD3DCreateParam* param )
{
    ...
    WritePrivateProfileString( "D3DADAPTER", "adapter", _itoa( param->adapter, buf, 10 ), file );
    WritePrivateProfileString( "D3DDEVTYPE", "devtype", _itoa( param->dev_type, buf, 10 ), file );
    WritePrivateProfileString( "BEHAVIOR", "behavior", _itoa( param->behavior_flag, buf, 10 ), file );
    // present_param
    WritePrivateProfileString( "PRESENT_PARAM", "windowed", _itoa( param->present_param.Windowed, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "backbuffer_count", _itoa( param->present_param.BackBufferCount, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "backbuffer_format", _itoa( param->present_param.BackBufferFormat, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "backbuffer_height", _itoa( param->present_param.BackBufferHeight, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "backbuffer_width", _itoa( param->present_param.BackBufferWidth, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "enable_depthstencil", _itoa( param->present_param.EnableAutoDepthStencil, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "depthstencil_format", _itoa( param->present_param.AutoDepthStencilFormat, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "multisample_type", _itoa( param->present_param.MultiSampleType, buf, 10 ), file );
    WritePrivateProfileString( "PRESENT_PARAM", "refresh_rate", _itoa( param->present_param.FullScreen_RefreshRateInHz, buf, 10 ), file );

#if ( defined LW_USE_DX8 )
    WritePrivateProfileString( "PRESENT_PARAM", "present_interval", _itoa( param->present_param.FullScreen_PresentationInterval, buf, 10 ), file );
#elif ( defined LW_USE_DX9 )
    WritePrivateProfileString( "PRESENT_PARAM", "present_interval", _itoa( param->present_param.PresentationInterval, buf, 10 ), file );
#endif
